...
print(f"[BEST] RF: numTrees={rf_best.getOrDefault(rf.numTrees)}, maxDepth={rf_best.getOrDefault(rf.maxDepth)}")
...
print(f"[BEST] DT: maxDepth={dt_best.getOrDefault(dt.maxDepth)}, impurity={dt_best.getOrDefault(dt.impurity)}")
...
print(f"[BEST] GBT: maxIter={gbt_best.getOrDefault(gbt.maxIter)}, maxDepth={gbt_best.getOrDefault(gbt.maxDepth)}")
...
print(f"[BEST] LR: regParam={lr_best.getOrDefault(lr.regParam)}, elasticNetParam={lr_best.getOrDefault(lr.elasticNetParam)}")
...
if HAS_XGBOOST:
    ...
    print(f"[BEST] XGBoost: max_depth={xgb_best.getOrDefault(xgb.max_depth)}, learning_rate={xgb_best.getOrDefault(xgb.learning_rate)}")
...
if HAS_LIGHTGBM:
    ...
    print(f"[BEST] LightGBM: numLeaves={lgbm_best.getOrDefault(lgbm.numLeaves)}, learningRate={lgbm_best.getOrDefault(lgbm.learningRate)}")
...
print(f"[BEST] MLP: layers={mlp_best.getOrDefault(mlp.layers)}, maxIter={mlp_best.getOrDefault(mlp.maxIter)}")
...
# RF
rf_best = cv_rf_model.bestModel.stages[-1]
rf_tuned = RandomForestClassifier(featuresCol=features_col, labelCol="label_binary", numTrees=rf_best.getOrDefault(rf.numTrees), maxDepth=rf_best.getOrDefault(rf.maxDepth), seed=42)
...
if HAS_XGBOOST:
    from shared_utils import SparkXGBClassifier
    xgb_best = cv_xgb_model.bestModel.stages[-1]
    xgb_tuned = SparkXGBClassifier(features_col=features_col, label_col="label_binary", max_depth=xgb_best.getOrDefault(xgb.max_depth), learning_rate=xgb_best.getOrDefault(xgb.learning_rate), num_workers=4)
...
if HAS_LIGHTGBM:
    from shared_utils import LightGBMClassifier
    lgbm_best = cv_lgbm_model.bestModel.stages[-1]
    lgbm_tuned = LightGBMClassifier(featuresCol=features_col, labelCol="label_binary", numLeaves=lgbm_best.getOrDefault(lgbm.numLeaves), learningRate=lgbm_best.getOrDefault(lgbm.learningRate), objective="binary")
