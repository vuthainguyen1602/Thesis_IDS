...
if os.path.exists(config_path):
    ...
    best_config = {
        ...
        "config": {"type": "feature_selection", "csv": ml_results_dir(ML02_DIR, "feature_importance.csv"), "top_k": 40, "col": "feature"}
    }
...
rf_grid = ParamGridBuilder() \
    .addGrid(rf.numTrees, [200] if FAST_MODE else [200, 300]) \
    .addGrid(rf.maxDepth, [15, 20]) \
    .addGrid(rf.minInstancesPerNode, [1] if FAST_MODE else [1, 5]) \
    .build()
start = time.time()
cv_rf = CrossValidator(estimator=pipeline_rf, estimatorParamMaps=rf_grid, evaluator=evaluator_cv, numFolds=CV_FOLDS, parallelism=4, seed=42)
cv_rf_model = cv_rf.fit(train_cv_df)
...
dt_grid = ParamGridBuilder() \
    ...
    .addGrid(dt.impurity, ["gini"] if FAST_MODE else ["gini", "entropy"]) \
    .addGrid(dt.minInstancesPerNode, [1, 5]) \
    .build()
start = time.time()
cv_dt = CrossValidator(estimator=pipeline_dt, estimatorParamMaps=dt_grid, evaluator=evaluator_cv, numFolds=CV_FOLDS, parallelism=4, seed=42)
cv_dt_model = cv_dt.fit(train_cv_df)
...
if RUN_GBT:
    ...
    gbt_grid = ParamGridBuilder() \
        .addGrid(gbt.maxIter, [100] if FAST_MODE else [100, 150]) \
        .addGrid(gbt.maxDepth, [6, 8]) \
        .addGrid(gbt.stepSize, [0.1] if FAST_MODE else [0.05, 0.1]) \
        .build()
    start = time.time()
    cv_gbt = CrossValidator(estimator=pipeline_gbt, estimatorParamMaps=gbt_grid, evaluator=evaluator_cv, numFolds=CV_FOLDS, parallelism=2, seed=42)
    cv_gbt_model = cv_gbt.fit(train_cv_df)
    ...
    print("\n[INFO] Skipping GBT tuning (fast mode). Set IDS_EXP2_FULL=1 or IDS_EXP2_GBT=1 to enable.")
...
lr_grid = ParamGridBuilder() \
    ...
    .addGrid(lr.elasticNetParam, [0.5] if FAST_MODE else [0.5, 0.8]) \
    .build()
start = time.time()
cv_lr = CrossValidator(estimator=pipeline_lr, estimatorParamMaps=lr_grid, evaluator=evaluator_cv, numFolds=CV_FOLDS, parallelism=4, seed=42)
cv_lr_model = cv_lr.fit(train_cv_df)
...
if RUN_EXTENDED and HAS_XGBOOST:
    ...
    xgb_grid = ParamGridBuilder() \
        ...
        .build()
    start = time.time()
    cv_xgb = CrossValidator(estimator=pipeline_xgb, estimatorParamMaps=xgb_grid, evaluator=evaluator_cv, numFolds=CV_FOLDS, parallelism=2, seed=42)
    cv_xgb_model = cv_xgb.fit(train_cv_df)
...
if RUN_EXTENDED and HAS_LIGHTGBM:
    ...
    lgbm_grid = ParamGridBuilder() \
        ...
        .build()
    start = time.time()
    cv_lgbm = CrossValidator(estimator=pipeline_lgbm, estimatorParamMaps=lgbm_grid, evaluator=evaluator_cv, numFolds=CV_FOLDS, parallelism=2, seed=42)
    cv_lgbm_model = cv_lgbm.fit(train_cv_df)
...
if RUN_EXTENDED:
    ...
    mlp_grid = ParamGridBuilder() \
        ...
        .build()
    start = time.time()
    cv_mlp = CrossValidator(estimator=pipeline_mlp, estimatorParamMaps=mlp_grid, evaluator=evaluator_cv, numFolds=CV_FOLDS, parallelism=4, seed=42)
    cv_mlp_model = cv_mlp.fit(train_cv_df)
...
if RUN_EXTENDED and HAS_XGBOOST and xgb_best_params:
    from shared_utils import SparkXGBClassifier
...
if RUN_EXTENDED and HAS_LIGHTGBM and lgbm_best_params:
    from shared_utils import LightGBMClassifier
...
plot_comparison(all_results, title="ml_03: Tuned Models Comparison", save_path=os.path.join(base_output, "comparison.png"), show=False)
plot_training_time(all_results, title="ml_03: Tuning Time", save_path=os.path.join(base_output, "train_time.png"), show=False)

plot_roc_curves(cv_models, test_df, title="ml_03: Tuned Models ROC Curves", save_path=os.path.join(base_output, "roc_curves.png"), show=False)
...
export_multi_section_report(report_sections, title=f"ml_03: Grid Search on {best_config['method_name']}", output_path=os.path.join(base_output, "report.html"))
