...
In the in-distribution stability check (a subset of the test set), the ranking of the methods \emph{is preserved} (XGBoost leads in all four methods); details in \texttt{robustness\_holdout\_summary.csv}. The multiclass evaluation (Figure~\ref{fig:cm}, Table~\ref{tab:perclass}) reveals a sharp contrast between weighted-F1 ($\approx$0.998) and macro-F1 ($\approx$0.806): the majority classes achieve high F1, while rare classes such as Web Attack (XSS, SQL Injection) and Bot have very low recall (0.03--0.46) --- where feature selection makes the clearest difference.
