...
\begin{tikzpicture}[
  ...
  \node[hl, below=of raw] (prep) {\textbf{Leakage-aware preparation} (Alg.~\ref{alg:prep})\\ drop \texttt{destination\_port}/\texttt{source\_port} $\cdot$ feature-level dedup};
  ...
  \node[box, below=of val] (edge) {SHAP Top-30 model $\rightarrow$ export to Jetson Orin Nano Super\\ (companion systems paper)};
  \draw[arr] (raw) -- (prep);
  \draw[arr] (prep) -- (split);
...
\begin{table*}[htbp]
  ...
  \vspace{2pt}
  {\footnotesize $^{\dagger}$Sizes are obtained by serializing each fitted model \emph{on the training cluster}; a distributed \texttt{save()} writes the tree-ensemble data partitions on whichever executor holds them, so entries in the KB range (here 0.003\,MB) captured only the driver-local metadata and \emph{under-report} the true size. Since the size of a tree ensemble is governed by the number of trees and nodes rather than by the number of input features, the SHAP Top-30 XGBoost model is in reality comparable to its RF Top-30 counterpart ($\approx$2.5\,MB). For the same reason, a reduced feature set can even yield a slightly \emph{larger} model than the baseline (2.52 vs.\ 2.32\,MB): with fewer split candidates the trees need more nodes to reach comparable purity. The single-node export of the deployed pipeline measures model sizes reliably.}
\end{table*}
...
\begin{table}[htbp]
  \caption{Multi-resampling stability and paired testing (values from \texttt{statistical\_validity\_multiseed.csv}).}
  \label{tab:stat}
  \centering
  \footnotesize
  \begin{tabular}{llcc}
