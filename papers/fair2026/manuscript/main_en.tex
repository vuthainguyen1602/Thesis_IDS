...
{\normalsize Thai Vu Nguyen}\\[3pt]
{\itshape\small University of Transport\\ and Communications\\ Ho Chi Minh City Campus}\\
{\small Ho Chi Minh City, Vietnam\\ nvthai@utc2.edu.vn}
\end{minipage}\hfill
\begin{minipage}[t]{0.24\textwidth}\centering
{\normalsize Dung Van Bui}\\[3pt]
{\itshape\small University of Information\\ Technology\\ VNU-HCM}\\
{\small Ho Chi Minh City, Vietnam\\ dungbv.17@grad.uit.edu.vn}
\end{minipage}\hfill
\begin{minipage}[t]{0.24\textwidth}\centering
{\normalsize Nhut Tri Do}\\[3pt]
{\itshape\small University of Information\\ Technology\\ VNU-HCM}\\
{\small Ho Chi Minh City, Vietnam\\ trinhutdo@uit.edu.vn}
\end{minipage}\hfill
\begin{minipage}[t]{0.24\textwidth}\centering
{\normalsize Van Du Nguyen}\\[3pt]
{\itshape\small Faculty of Information\\ Technology\\ Nong Lam University}\\
{\small Ho Chi Minh City, Vietnam\\ nvdu@hcmuaf.edu.vn}
\end{minipage}%
}
...
\textbf{In-distribution stability check.} The representative model of each method is re-evaluated on a subset of the test set (disjoint from the training set). This is only an \emph{in-distribution stability} check; the true distribution-shift assessment is the cross-dataset experiment (Section~\ref{sec:exp}).
...
\begin{figure}[htbp]
  ...
  \IfFileExists{../../../results/ml_10_leakage_ablation/leakage_ablation.png}{%
    \includegraphics[width=0.74\columnwidth]{leakage_ablation.png}%
  }{%
...
\begin{table*}[htbp]
  ...
  \vspace{2pt}
  {\scriptsize $^{\dagger}$Sizes are obtained by serializing each fitted model \emph{on the training cluster}; the distributed model-saving step writes the tree-ensemble data partitions on whichever executor holds them, so entries in the KB range (here 0.003\,MB) captured only the driver-local metadata and \emph{under-report} the true size. Since the size of a tree ensemble is governed by the number of trees and nodes rather than by the number of input features, the SHAP Top-30 XGBoost model is in reality comparable to its RF Top-30 counterpart ($\approx$2.5\,MB). For the same reason, a reduced feature set can even yield a slightly \emph{larger} model than the baseline (2.52 vs.\ 2.32\,MB): with fewer split candidates the trees need more nodes to reach comparable purity. The single-node export of the deployed pipeline measures model sizes reliably.}
\end{table*}
