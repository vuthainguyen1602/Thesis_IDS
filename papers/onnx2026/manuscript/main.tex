...
(4) An open, archived software artifact\footnote{Zenodo concept DOI: \url{https://doi.org/10.5281/zenodo.22731927}.} and reproducible benchmark scripts.
...
\textbf{Serving classical ML on constrained hardware.} ONNX\footnote{\url{https://onnx.ai/}} standardizes a small operator set, including \texttt{TreeEnsembleClassifier} for decision-tree ensembles, so a model trained in one framework can be served by any ONNX-compatible runtime; ONNX Runtime\footnote{\url{https://onnxruntime.ai/}} is the reference CPU/GPU implementation and is what we target as the primary backend. Nakandala et al.~\cite{nakandala2020hummingbird} make a related argument for tree ensembles specifically: compiling scikit-learn-style pipelines into tensor operations lets a GPU tensor runtime, rather than a tree-optimized one, serve them, with large reported speedups over native execution on batched workloads. Our NumPy fallback takes the opposite trade-off deliberately: no compilation step and no GPU dependency, at the cost of the batch-size sensitivity Sect.~\ref{sec:results} measures. Koschel et al.~\cite{koschel2023arm} show that this cost is not fundamental to ARM hardware: porting the QuickScorer tree-traversal algorithm from Intel AVX to ARM NEON yields up to $9.4\times$ speedups for ensemble inference on IoT-class ARM devices, with fixed-point quantization improving throughput further at a small accuracy cost. That result targets exactly the traversal loop our NumPy backend currently runs unvectorized-per-instruction-but-batched; we return to it as a concrete optimization target in Sect.~\ref{sec:discussion}.
...
Because the NumPy backend vectorizes tree traversal across the rows of one batch rather than compiling per-tree kernels, its throughput scales strongly with batch size: 9\,rows/s at batch 1, 150\,rows/s at batch 20, and 1{,}650\,rows/s at batch 500 on the same board. ONNX Runtime is comparatively insensitive to batch size over the same range (Sect.~\ref{sec:discussion}). This means the two backends should not be compared at a single fixed batch size without stating it, and a deployment choosing the NumPy fallback should size its batch/linger policy accordingly rather than default to a small batch tuned for the ONNX path.
...
\textbf{Applying recent tree-ensemble-on-ARM work to the NumPy backend.} The NumPy backend's batch-size sensitivity (Sect.~\ref{sec:results}) is a direct symptom of walking each tree with masked NumPy operations rather than a hardware-tuned traversal. Koschel et al.'s ARM NEON port of QuickScorer~\cite{koschel2023arm} targets exactly this loop and reports up to $9.4\times$ speedups on ARM IoT hardware with fixed-point quantization; applying that bitvector traversal scheme to \texttt{NumpyInferenceEngine} is a concrete, scoped next step rather than a vague ``optimize later,'' and would plausibly narrow the batch-size sensitivity as well as the steady-state gap to ONNX Runtime.
