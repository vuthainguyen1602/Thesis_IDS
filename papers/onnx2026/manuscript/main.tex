...
Table~\ref{tab:engines} reproduces the companion paper's single-node microbenchmark of the same Random Forest and the same 29-feature input across three engines, measured on the classifier board at batch size 10; it motivates this paper rather than reports a new measurement, and is included so the two papers can be read independently. ONNX Runtime already reaches $348\times$ the deployed Spark throughput and roughly 2\% of scikit-learn's energy per inference in this single-node setting, before any pipeline integration.

\begin{table}[htbp]
  \caption{Single-node inference cost of the same Random Forest and feature set across engines (companion microbenchmark, reproduced for reference).}
  \label{tab:engines}
  \centering
  \small
  \begin{tabular}{l c c c c}
    ...
    \bottomrule
  \end{tabular}
  \tabnote{Source: \texttt{papers/soict2026/results/benchmarks/benchmark\_engines\_results.json}, single-node baseline, batch size 10, 5{,}000 samples per engine. The scikit-learn and ONNX Runtime rows are the scikit-learn-exported reference used to validate the conversion path prior to the direct Spark-Parquet export described in Sect.~\ref{sec:export}; the PySpark row is the deployed classifier board's own measured cost from the same study.}
\end{table}
...
The single-node microbenchmark (Table~\ref{tab:engines}) already establishes that the JVM, not the model, is the dominant edge-serving cost for this system: the same forest is $9\times$ faster under scikit-learn and $348\times$ faster under ONNX Runtime, and neither of those engines was, until this paper, wired into the streaming pipeline at all. Closing that gap required more than pointing a converter at the model: because the deployed forest was selected under a leakage-aware protocol specifically to avoid the optimistic scores that leakage and dataset artifacts produce~\cite{arp2022dosdonts,lanvin2023errors}, any export path that silently retrained or approximated the model would have undermined that selection. Reading Spark's own Parquet node table directly, rather than going through a scikit-learn intermediate, is what lets us claim the ONNX and NumPy backends serve \emph{the same} model rather than a plausible substitute -- a claim Sect.~\ref{sec:results} still needs to close with a measured agreement rate rather than the equivalence argument alone.
