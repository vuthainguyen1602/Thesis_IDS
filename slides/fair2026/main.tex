...
\begin{frame}{Result 2: testing the difference instead of asserting it}
  \begin{columns}[T]
    ...
    \end{column}
  \end{columns}
  \takeaway{Significant \emph{only at the resolution floor}, gap $\approx$0.0001:
  \key{practically equivalent}, so explainability and cost decide. After dedup the
  port's isolated effect is small --- duplicates were the dominant leak.}
...
\begin{frame}{Result 4: the scores do not survive a different testbed}
  \begin{columns}[T]
    \begin{column}{0.46\textwidth}
      ...
      \begin{tabular}{lcc}
        ...
        \midrule
        2017 $\to$ 2017 & \good{0.9954} & \good{0.9997} \\
        2017 $\to$ 2018 & \bad{0.2154} \tiny$\pm$0.2562 & \bad{0.4989} \\
        2018 $\to$ 2018 & \good{0.9243} & \good{0.9675} \\
        2018 $\to$ 2017 & \bad{0.0536} \tiny$\pm$0.0116 & \bad{0.1862} \\
        \bottomrule
      \end{tabular}
      }
      \\[0.4em]
      \footnotesize
      Mean of 4 runs; shared leakage-free features, $\sim$2.39M flows.
    \end{column}
    \begin{column}{0.50\textwidth}
      \begin{itemize}\small
        \item In-domain stable \emph{both} ways --- cross-dataset AUC-PR
              \bad{1.00$\to$0.50} and \bad{0.97$\to$0.19}.
        \item Recall 0.126 / 0.030 at precision 0.850 / 0.248: the model
              \key{fails to recognize} the other testbed's attacks.
        \item Cross-dataset F1 \key{not reproducible} (0.045--0.30) --- report
              means over runs, judge on AUC-PR.
        \item Causes: CICFlowMeter version, network configuration, attack
              tooling.
      \end{itemize}
    \end{column}
  \end{columns}
  ...
  \note{Do not soften this. In-domain 0.99, cross-dataset AUC-PR 0.50 / 0.19.
  If asked about the wide F1 interval: that is the finding --- at this recall the
  metric is threshold-fragile, which is why we report means over runs and read
  the gap off AUC-PR. Consistent with prior cross-dataset IDS studies, and the
  reason domain adaptation is a prerequisite rather than a nice-to-have.}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \begin{itemize}
        ...
        \item Cross-dataset AUC-PR \bad{collapses to 0.50 / 0.19}.
      \end{itemize}
    \end{column}
