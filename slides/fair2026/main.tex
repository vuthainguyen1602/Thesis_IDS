...
%   III Results, conclusion ........ 3:50   (5 slides)
...
\begin{frame}{A near-perfect F1 on CICIDS2017 is a warning sign}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      ...
      \begin{enumerate}
        ...
        \item \bad{Feature-level duplicates} survive full-row dedup --- the
              same sample lands in train \emph{and} test.
      \end{enumerate}
    \end{column}
  ...
  port-to-label shortcut; 80 features are expensive to train, to score and to
...
\begin{frame}{Result 2: testing the difference instead of asserting it}
  ...
  {\tiny\textcolor{gray!55!black}{Nadeau \& Bengio, \emph{Inference for the
  Generalization Error}, Machine Learning 52(3), 2003.}}
  \note{Variance reflects data sampling, not just model initialisation. The
  test is two-sided, and overlapping resamplings break t-independence.
  Small p, tiny effect in absolute terms: say plainly that we treat them as
  equivalent rather than claiming a winner.}
\end{frame}
...
\begin{frame}{Key references}
  \begin{itemize}\footnotesize
    ...
    \item Lanvin, M. et al. \emph{Errors in the CICIDS2017 Dataset and the
          Significant Differences in Detection Performances It Makes}.
          CRiSIS 2022.
          \hfill{\key{label conflicts}}
    \item Arp, D. et al. \emph{Dos and Don'ts of Machine Learning in Computer
    ...
    \item Lundberg, S.\,M. \& Lee, S.-I. \emph{A unified approach to
          ...
          \hfill{\key{SHAP}}
    \item Nadeau, C. \& Bengio, Y. \emph{Inference for the Generalization
          Error}. Machine Learning 52(3), 2003.
          \hfill{\key{corrected CI}}
    \item D'hooge, L. et al. \emph{Inter-dataset generalization strength of
    ...
    \item Zaharia, M. et al. \emph{Spark: Cluster computing with working
          sets}. HotCloud 2010.
          \hfill{\key{Spark ML}}
    \item CIC \& CSE. \emph{CSE-CIC-IDS2018 dataset}. 2018.
          \hfill{\key{cross-dataset target}}
  \end{itemize}
  \note{Not presented --- shown while taking questions, so sources are on
  screen if the committee asks. Full list is in the paper.}
\end{frame}
