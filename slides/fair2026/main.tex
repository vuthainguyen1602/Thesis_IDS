...
\begin{frame}{Result 3: binary F1 hides the rare classes}
  \begin{columns}[c]
    ...
    \end{column}
  \end{columns}
  \note{Weighted F1 0.998 versus macro F1 0.806. Three causes: extreme
  imbalance; removing the port hits web attacks hardest, because without the
  port-80 shortcut short HTTP flows look like browsing, so the 0.027 recall
  reported here replaces a leakage-inflated one; and Bot C\&C beaconing
  resembles background traffic.}
\end{frame}
