...
\begin{frame}{Split-deployment architecture}
  ...
  \begin{tikzpicture}[
    ...
    \draw[arr] (j1.east) -- node[lbl]{suspicious-flow topic\\ ($\approx$4\% of flows, mode A)} (j2.west);
  ...
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      \begin{itemize}
        ...
        \item \bad{B stays Spark-bound} ($\approx$2.3$\times$): every node still classifies.
      \end{itemize}
    \end{column}
    \begin{column}{0.49\textwidth}
      \begin{itemize}
        \item C is fastest on average but swings by $\pm$25.6 flows/s and needs the host master.
        \item Energy amortized over 24--35$\times$ more verdicts:
              \good{$\approx$2.5\,J $\rightarrow$ 0.12--0.18\,J}..
      \end{itemize}
    \end{column}
  \end{columns}
...
\begin{frame}{Backup: measurement protocol}
  \begin{itemize}\small
    ...
    \item End-to-end send-to-verdict latency from producer timestamps under
          NTP-synchronized clocks (excludes DB-write cost).
