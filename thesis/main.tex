% Define metadata
\documentclass{template/thesis}
\Title{PHÁT HIỆN XÂM NHẬP (IDS) DỰA TRÊN APACHE SPARK CHO BẢO MẬT MẠNG IoT}
\Title[en]{INTRUSION DETECTION (IDS) BASED ON APACHE SPARK FOR IoT NETWORK SECURITY}

\ThesisType{LUẬN VĂN THẠC SĨ}

\University{ĐẠI HỌC QUỐC GIA THÀNH PHỐ HỒ CHÍ MINH}
\CollegeOrDept{{TRƯỜNG ĐẠI HỌC CÔNG NGHỆ THÔNG TIN}}


\Author{BÙI VĂN DŨNG}{220201014}

\Degree{THẠC SĨ}
\Major{CÔNG NGHỆ THÔNG TIN}
% \MajorCode{123}
\ThesisYear{2026}
\ThesisLocation{TP. HỒ CHÍ MINH}

\Supervisor{TS. NGUYỄN VĂN DŨ}
\Supervisor{TS. ĐỖ TRÍ NHỰT}

\hypersetup{
	pdfencoding=auto,
	pdfauthor={Bùi Văn Dũng},
	pdfsubject={Phát hiện xâm nhập dựa trên Apache Spark cho bảo mật mạng IoT},
	pdftitle={Phát hiện xâm nhập (IDS) dựa trên Apache Spark cho bảo mật mạng IoT},
	pdfkeywords={IDS, Apache Spark, IoT, Machine Learning, CICIDS2017}
}

% Placeholder cho số liệu sẽ điền sau khi chạy lại pipeline (leak-free).
% Thay mỗi \ph{...} bằng giá trị từ results/*.csv tương ứng.
\newcommand{\ph}[1]{{\normalfont\itshape\color{gray}[#1]}}

% Chú giải ký hiệu đặt DƯỚI bảng (caption chỉ giữ một câu mô tả). Minipage hủy
% \centering của môi trường table để khối chú giải được canh đều thay vì canh giữa
% từng dòng. \relax kết thúc glue spec, nếu thiếu TeX sẽ quét tiếp từ khóa "fil(l)"
% và nuốt mất chữ "l"/"L" đứng đầu nội dung chú giải.
\newcommand{\tabnote}[1]{%
  \vspace{4pt}\par\noindent
  \begin{minipage}{\textwidth}%
    \footnotesize\parindent=0pt\leftskip=0pt\rightskip=0pt\parfillskip=0pt plus 1fil\relax
    #1%
  \end{minipage}}

% TikZ libraries cho các sơ đồ (tikz + calc đã được nạp trong thesis.cls).
\usetikzlibrary{positioning, arrows.meta, shapes.geometric, fit, backgrounds, calc}

% \renewcommand{\algorithmicrequire}{\textbf{Đầu vào:}}
% \renewcommand{\algorithmicensure}{\textbf{Đầu ra:}}

% \renewcommand{\algorithmicrequire}{\textbf{Đầu vào:}}
% \renewcommand{\algorithmicensure}{\textbf{Đầu ra:}}

% Okabe-Ito colorblind-safe accents for TikZ diagrams
\definecolor{accBlue}{HTML}{0072B2}
\definecolor{accGreen}{HTML}{009E73}
\definecolor{accRed}{HTML}{C1121F}

\begin{document}
\firstbeforepreface
\secondbeforepreface
\setcounter{page}{1}
\newpage
\afterpreface

\chapter{Mở đầu}
\label{c:intro}
\input{chapters/1-MoDau}

\chapter{Cơ sở lý thuyết}
\label{c:cosolythuyet}
\input{chapters/2-CoSoLyThuyet}


\chapter{Phương pháp thực hiện}
\label{c:phuongphapthuchien}
\input{chapters/3-PhuongPhapThucHien}

\chapter{Thực nghiệm, đánh giá và thảo luận}
\label{c:thucnghiemdanhgia}
\input{chapters/4-ThucNghiemDanhGia}


\chapter{Kết luận và hướng phát triển}
\label{c:ketluanvaphattrien}
\input{chapters/5-KetLuanPhatTrien}


\appendix
\renewcommand{\cftchappresnum}{Phụ lục }
\addtocontents{toc}{\protect\renewcommand{\protect\cftchappresnum}{Phụ lục }}
\input{chapters/appendix}

\begingroup
\setlength{\emergencystretch}{8em}
\justifying\printbibliography[title=Tài liệu tham khảo, heading=bibintoc]
\endgroup

% Bản PDF bài báo FAIR 2026 đính kèm ở cuối luận văn
\IfFileExists{../output/pdfs/FAIR2026.pdf}{%
  \includepdf[pages=-,scale=0.9,fitpaper=false,pagecommand={\thispagestyle{empty}}]{../output/pdfs/FAIR2026.pdf}%
}{}



\end{document}
