...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        \node[io]  (data)  at (0, 0.0)  {\textbf{(1) Thu thập \& Tiền xử lý} --- CICIDS2017\\
            loại trừ rò rỉ dữ liệu (bỏ \texttt{destination\_port}, khử trùng lặp đặc trưng), chia 80/20};
...
\State $D \gets \text{DropDuplicates}(D, \text{subset}=\mathcal{F})$ \Comment{Khử trùng lặp ở mức đặc trưng trước khi chia}
...
Tập dữ liệu CIC-\ac{ids}2017 được phân chia theo tỷ lệ 80/20 cho huấn luyện và kiểm tra, sử dụng chiến lược \textit{stratified split} để giữ nguyên phân bố nhãn. Các chỉ số hiệu năng được \textbf{công bố trên tập kiểm tra} (tập giữ lại cuối cùng, không tham gia huấn luyện, xếp hạng đặc trưng hay tinh chỉnh siêu tham số); kết quả trên tập huấn luyện chỉ được dùng để chẩn đoán hiện tượng quá khớp (overfitting) hoặc thiếu khớp (underfitting).


\section{Lựa chọn đặc trưng và giảm chiều dữ liệu}

Trong quá trình tiền xử lý dữ liệu cho bài toán phát hiện xâm nhập mạng, lựa chọn đặc trưng bao gồm \textbf{chọn đặc trưng} và \textbf{trích xuất đặc trưng} là một bước quan trọng nhằm giảm chiều dữ liệu, loại bỏ những đặc trưng không cần thiết, và cải thiện hiệu suất của mô hình học máy. Dữ liệu số được gom vào vector đặc trưng bằng \texttt{VectorAssembler} (77 thuộc tính số) và được chuẩn hóa bằng \texttt{StandardScaler} (chuẩn hóa Z-score). Ba phương pháp được sử dụng trong luận văn là \textit{\ac{rf}}, \ac{shap} và \ac{pca}.
...
\For{mỗi mẫu $x_j \in D_{\text{test}}$}
    \State $\bar{p}_j \gets \frac{1}{3} \displaystyle\sum_{s=1}^{3} P^{(s)}(y=1|x_j)$ \Comment{Tính xác suất trung bình lớp Attack}
    \If{$\bar{p}_j \geq 0.5$}
