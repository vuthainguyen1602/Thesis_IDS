...
\State \textbf{Bước 4b: Loại bỏ trùng lặp ở mức đặc trưng (tất định)}
...
\For{mỗi nhóm $g \in G$}
    \If{$|\{y_i : (x_i, y_i) \in g\}| > 1$} \Comment{Nhóm mâu thuẫn nhãn (Benign lẫn Attack)}
        \State Loại toàn bộ nhóm $g$ khỏi $D$; ghi log số nhóm/hàng đụng độ
    \Else
...
\begin{table}[htbp]
\centering
\caption{Các phương pháp học máy tổng hợp (ensemble) trong Giai đoạn 3}
\label{tab:ensemble}
\renewcommand{\arraystretch}{1.1}
