...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        dep/.style={draw=accRed,thick, rounded corners, fill=accRed!10,text width=8.6cm, minimum height=1.0cm, inner sep=3pt},
        arr/.style ={-{Stealth[length=2.5mm]}, thick, draw=gray!60!black},
        loop/.style={-{Stealth[length=2.5mm]}, thick, draw=accRed, dashed},
    ]
        ...
        \node[anchor=west, font=\footnotesize, text width=2.6cm, text=accRed!85!black]
...
\Statex \textcolor{accRed}{\textbf{$\rightarrow$ Hai bước loại trừ rò rỉ dữ liệu (data leakage): Bước 4 và Bước 4b}}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        \node[box, draw=accRed,      fill=accRed!12, text width=3.1cm] (prep)  at (4.2,0)    {Tiền xử lý\\\textbf{+ loại trừ rò rỉ}\\(Thuật toán~\ref{data_preparation_ag})};
        ...
        \node[box, draw=accRed, fill=accRed!16, text width=3.3cm] (test)  at (12.4,-1.4) {\textbf{Test} (20\%)\\{\footnotesize giữ lại, chỉ công bố cuối}};
    ...
    \end{tikzpicture}}
    \caption[Sơ đồ phân chia dữ liệu CICIDS2017]{Sơ đồ phân chia dữ liệu CICIDS2017 (train/test 80/20, \texttt{seed}=42, trong đó train tách tiếp 80/20 thành base-train/validation)}
...
Trong quá trình tiền xử lý dữ liệu cho bài toán phát hiện xâm nhập mạng, lựa chọn đặc trưng bao gồm \textbf{chọn đặc trưng} và \textbf{trích xuất đặc trưng} là một bước quan trọng nhằm giảm chiều dữ liệu, loại bỏ những đặc trưng không cần thiết, và cải thiện hiệu năng của mô hình học máy \cite{guyon2003introduction}. Dữ liệu số được gom vào vector đặc trưng bằng \texttt{VectorAssembler} (khoảng 78 đặc trưng số trước khi loại cổng, chính xác là 77 đặc trưng sau khi loại \texttt{destination\_port}/\texttt{source\_port}) và được chuẩn hóa bằng \texttt{StandardScaler} (chuẩn hóa Z-score). Ba phương pháp được sử dụng trong luận văn là \textit{\ac{rf}}, \ac{shap} và \ac{pca}.
...
\begin{enumerate}[label=\alph*)]
    ...
    Các đặc trưng có giá trị \ac{shap} lớn (theo độ lớn tuyệt đối) được xem là có ảnh hưởng quan trọng hơn đến quyết định của mô hình. Phương pháp này được ứng dụng rộng rãi trong giải thích mô hình phức tạp như XGBoost, Random Forest, và mạng nơ-ron \cite{lundberg2017unified}. 
...
Việc áp dụng kỹ thuật Voting cho phép khai thác sự đa dạng giữa các mô hình học cơ sở, từ đó giảm sai số do thiên lệch của từng mô hình riêng lẻ và cải thiện khả năng tổng quát hóa trên dữ liệu chưa từng thấy. Quy trình chi tiết được trình bày trong Thuật toán~\ref{voting_ag}.
...
Cách tiếp cận này cho phép giảm phương sai của các mô hình cơ sở nhờ cơ chế lấy mẫu bootstrap, đồng thời duy trì khả năng học biểu diễn của từng thuật toán riêng lẻ. Việc chọn lọc các mô hình có hiệu năng cao nhất trước khi áp dụng Bagging nhằm cân bằng giữa độ chính xác và tính ổn định của hệ thống. Chi tiết quy trình được trình bày trong Thuật toán~\ref{bagging_ag}.

\begin{algorithm}[htbp]
\caption{Hybrid Bagging Ensemble Learning}
\label{bagging_ag}
\begin{algorithmic}[1]
...
Mỗi mô hình (cơ sở và ensemble) được đánh giá trên bốn chiều:
...
\midrule
Độ chính xác (Accuracy)         & Tỷ lệ dự đoán đúng trên toàn bộ mẫu kiểm tra \\
Precision                        & Tỷ lệ cảnh báo đúng trong tổng số cảnh báo được phát ra \\
Recall                           & Tỷ lệ phát hiện đúng trong tổng số mẫu tấn công thực tế \\
...
Hiệu năng của mô hình được đánh giá thông qua các chỉ số phân loại phổ biến như Accuracy, Precision, Recall và F1-Score, kết hợp với phân tích đường cong AUC (ROC và PR), chi phí tính toán (thời gian huấn luyện và suy luận), cũng như khả năng giải thích của mô hình (ví dụ: biểu đồ tổng quan SHAP). Các tiêu chí đánh giá được tổng hợp trong Bảng~\ref{tab:evaluation}. Để kiểm chứng mô hình phát hiện xâm nhập, tập dữ liệu kiểm tra được sử dụng nhằm đánh giá khả năng tổng quát hóa của mô hình đã huấn luyện, trong đó kết quả dự đoán được đối chiếu với nhãn thực tế và đo lường bằng các thước đo đánh giá. Các chỉ số như \ac{ac}, \ac{pr}, \ac{re}, \ac{fs} được tính toán theo các công thức~\eqref{eq:accuracy}, \eqref{eq:precision}, \eqref{eq:recall} và~\eqref{eq:f1score}. Những kết quả này là cơ sở để so sánh giữa các phương pháp lựa chọn đặc trưng và lựa chọn mô hình phù hợp cho triển khai thực tế.
...
\clearpage
\section{Tối ưu hóa siêu tham số và huấn luyện mô hình}
...
\begin{enumerate}
    ...
    \item Chọn mô hình tốt nhất dựa trên điểm đánh giá trung bình trên các fold.
\end{enumerate}
...
Quy trình này bảo đảm chọn được tổ hợp siêu tham số tốt nhất trong lưới đã định nghĩa, và Spark rút ngắn thời gian tìm kiếm nhờ xử lý phân tán, song song hóa trên cụm máy.
