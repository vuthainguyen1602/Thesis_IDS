...
Trong chương này, luận văn sẽ trình bày phương pháp đề xuất để xây dựng mô hình phát hiện bất thường dựa trên Apache Spark cho bảo mật mạng \ac{iot}. Quá trình thực hiện bao gồm 6 bước chính, được thể hiện qua Hình~\textbf{\ref{fig:trainning_model}}: (1) Thu thập dữ liệu và tiền xử lý; (2) Xây dựng đặc trưng và lựa chọn đặc trưng; (3) Huấn luyện mô hình; (4) Đánh giá mô hình; (5) Tối ưu siêu tham số; và (6) Triển khai biên thời gian thực. Các bước (1)–(5) được lặp lại tương ứng với từng phương pháp lựa chọn đặc trưng (\ac{rfi}, \ac{shap}, \ac{pca}), sau đó lựa chọn mô hình tốt nhất để triển khai trên thiết bị biên.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        ml/.style ={draw=blue!55,        thick, rounded corners, fill=blue!5,   text width=8.6cm, minimum height=1.0cm, inner sep=3pt},
        dep/.style={draw=orange!75!black,thick, rounded corners, fill=orange!10,text width=8.6cm, minimum height=1.0cm, inner sep=3pt},
        arr/.style ={-{Stealth[length=2.5mm]}, thick, draw=gray!60!black},
        loop/.style={-{Stealth[length=2.5mm]}, thick, draw=orange!75!black, dashed},
    ]
...
Số loại nhãn                     & 14 loại (tấn công và bình thường) \\
...
Bên cạnh đó, để bảo đảm kết quả đánh giá phản ánh đúng năng lực mô hình, luận văn áp dụng thêm hai bước \emph{loại trừ rò rỉ dữ liệu} ngay trong giai đoạn tiền xử lý, trước khi phân chia tập: (i) \textbf{loại đặc trưng rò rỉ nhãn} \texttt{destination\_port} (và \texttt{source\_port} nếu có) — vì trong CICIDS2017, mỗi loại tấn công được sinh nhằm vào những cổng dịch vụ cố định nên cổng đích gần như mã hóa trực tiếp nhãn, khiến mô hình ``ghi nhớ'' ánh xạ cổng~$\rightarrow$~nhãn thay vì học hành vi luồng; và (ii) \textbf{loại bỏ trùng lặp ở mức đặc trưng} — loại các luồng giống nhau trên toàn bộ cột đặc trưng (không chỉ khử các bản ghi trùng lặp hoàn toàn), tránh việc cùng một mẫu xuất hiện ở cả tập huấn luyện lẫn kiểm tra khi phân chia ngẫu nhiên. Đối với tập dữ liệu CICIDS2017, quá trình tiền xử lý dữ liệu được thể hiện chi tiết ở \textbf{\textit{Thuật toán \ref{data_preparation_ag}}}.
...
\State \textbf{Bước 4: Xác định các đặc trưng số (đã loại trừ rò rỉ)}
...
\State $D \gets \text{DropDuplicates}(D, \text{subset}=\mathcal{F})$ \Comment{Loại bỏ trùng lặp ở mức đặc trưng trước khi chia}
...
Trong quá trình tiền xử lý dữ liệu cho bài toán phát hiện xâm nhập mạng, lựa chọn đặc trưng bao gồm \textbf{chọn đặc trưng} và \textbf{trích xuất đặc trưng} là một bước quan trọng nhằm giảm chiều dữ liệu, loại bỏ những đặc trưng không cần thiết, và cải thiện hiệu suất của mô hình học máy. Dữ liệu số được gom vào vector đặc trưng bằng \texttt{VectorAssembler} (khoảng 78 thuộc tính số trước khi loại cổng; số đặc trưng chính xác sau khi loại \texttt{destination\_port}/\texttt{source\_port} là \ph{}) và được chuẩn hóa bằng \texttt{StandardScaler} (chuẩn hóa Z-score). Ba phương pháp được sử dụng trong luận văn là \textit{\ac{rf}}, \ac{shap} và \ac{pca}.
...
Logistic Regression             & Chuẩn hoá L2                        & Tối ưu hàm mất mát logistic \\
Linear \ac{svm}                 & SVC tuyến tính                      & Tối đa hoá biên phân loại \\
Naive Bayes                     & Phân phối Gaussian                  & Ước lượng xác suất hậu nghiệm \\
Random Forest                   & Bagging                             & Tổ hợp cây quyết định ngẫu nhiên \\
\ac{gbt}                        & Gradient Boosting                   & Tối thiểu hoá hàm mất mát theo bậc \\
...
trên tập kiểm tra để xác định top-3 mô hình tốt nhất phục vụ bước tổ hợp tiếp theo.

\pagebreak
\subsection{Các phương pháp Ensemble Learning}
\begin{table}[htbp]
\centering
\caption{Các phương pháp tổ hợp mô hình trong Giai đoạn 3}
\label{tab:ensemble}
\renewcommand{\arraystretch}{1.1}
...
Majority Voting                 & Hard voting trên Top-3 mô hình theo F1-Score \\
                                 & Nhãn được chọn theo phiếu đa số từ ba bộ phân loại \\
\bottomrule
\end{tabular}
\end{table}
...
Sau khi hoàn tất giai đoạn huấn luyện đơn mô hình, luận văn tiến hành xây dựng mô hình kết hợp dựa trên kỹ thuật Voting để nâng cao độ chính xác và tính ổn định của hệ thống phân loại. Cụ thể, ba mô hình phân loại nhị phân có chỉ số F1-Score cao nhất ở giai đoạn trước sẽ được lựa chọn để tham gia vào quá trình ensemble.

Mỗi mô hình trong tập được chọn sẽ đưa ra dự đoán độc lập cho từng mẫu trong tập kiểm tra. Các dự đoán này sau đó được tổng hợp thông qua cơ chế bầu chọn. Trong trường hợp Majority Voting, nhãn cuối cùng của mỗi mẫu được xác định dựa trên lớp nhận được nhiều phiếu bầu nhất từ các mô hình thành phần. Đối với bài toán phân loại nhị phân, mẫu được gán nhãn \textit{Attack} nếu số lượng mô hình dự đoán lớp 1 lớn hơn một nửa tổng số mô hình, ngược lại sẽ được gán nhãn \textit{Benign}.

Việc áp dụng kỹ thuật Voting cho phép khai thác sự đa dạng giữa các mô hình học cơ sở, từ đó giảm sai số do thiên lệch của từng mô hình riêng lẻ và cải thiện khả năng tổng quát hóa trên dữ liệu chưa từng thấy. Nhờ cơ chế kết hợp này, hệ thống có thể nâng cao hiệu suất tổng thể trong việc phát hiện và phân loại chính xác các hành vi xâm nhập mạng. Quy trình chi tiết được trình bày trong \textbf{\textit{Thuật toán \ref{voting_ag}}}.
...
\For{$k = 1$ to $K$}
    ...
    \State $F_k \gets \text{F1-Score}(M_k, D_{\text{test}})$ \Comment{Tính điểm F1 của mô hình $M_k$ trên tập kiểm tra}
\EndFor
...
\State \textbf{Bước 4: Kết hợp bằng bầu chọn đa số (Hard/Majority Voting)}
\For{mỗi mẫu $x_j \in D_{\text{test}}$}
    \State $S_j \gets \displaystyle\sum_{s=1}^{3} \hat{y}_s^{(j)}$ \Comment{Tổng số phiếu dự đoán Attack}
    \If{$S_j > 3/2$}
        \State $y_{\text{vote}}^{(j)} \gets 1$ \Comment{Đa số phiếu: Attack}
    \Else
        \State $y_{\text{vote}}^{(j)} \gets 0$ \Comment{Đa số phiếu: Benign}
    \EndIf
\EndFor
...
Để khai thác hiệu quả sức mạnh của kỹ thuật học máy tổng hợp Bagging, luận văn đề xuất xây dựng một mô hình ensemble lai (Hybrid Bagging Ensemble) dựa trên các mô hình cơ sở có hiệu suất cao nhất ở giai đoạn 1. Cụ thể, thay vì chỉ sử dụng một mô hình đơn lẻ, quy trình bắt đầu bằng việc huấn luyện và đánh giá $K$ thuật toán học cơ sở trên tập dữ liệu huấn luyện $D$. Hiệu năng của từng mô hình được đo lường thông qua chỉ số F1-Score, từ đó lựa chọn ba mô hình có kết quả tốt nhất để đưa vào giai đoạn kết hợp.

Sau khi xác định Top-3 mô hình, phương pháp Bagging được áp dụng theo phân bố bản sao $(3,2,2)$. Đối với mỗi mô hình trong ba mô hình được chọn, nhiều tập dữ liệu bootstrap được tạo ra bằng cách lấy mẫu ngẫu nhiên có hoàn lại từ tập huấn luyện ban đầu. Trên mỗi tập bootstrap này, một bản sao của mô hình tương ứng được huấn luyện độc lập, tạo thành các \textit{bộ học cơ sở (base learners)} trong ensemble. Mỗi base learner được gán trọng số tỷ lệ với F1-Score của mô hình gốc nhằm phản ánh mức độ tin cậy tương đối của nó trong quá trình tổng hợp. Việc phân bổ bất đối xứng $(3,2,2)$ --- cấp nhiều bản sao bootstrap hơn cho mô hình hạng cao --- là một lựa chọn thiết kế thực nghiệm nhằm thiên ensemble về phía các bộ học mạnh hơn trong khi vẫn giữ tính đa dạng từ ba thuật toán khác loại; đây là một heuristic, không phải kết quả của tối ưu hoá lý thuyết, và các tỷ lệ khác (chẳng hạn $3$-$3$-$3$) có thể được khảo sát trong nghiên cứu tiếp theo.
...
Mỗi mô hình (cơ sở và tổ hợp) được đánh giá toàn diện trên bốn chiều:

\begin{table}[htbp]
\centering
\caption{Các tiêu chí đánh giá mô hình trong Giai đoạn 4}
...
Hiệu năng của mô hình được đánh giá thông qua các chỉ số phân loại phổ biến như Accuracy, Precision, Recall và F1-Score, kết hợp với phân tích đường cong AUC (ROC và PR), chi phí tính toán (thời gian huấn luyện và suy luận), cũng như khả năng giải thích của mô hình (ví dụ: biểu đồ tổng quan SHAP). Để xác thực mô hình phát hiện xâm nhập, tập dữ liệu kiểm tra được sử dụng nhằm đánh giá khả năng tổng quát hóa của mô hình đã huấn luyện, trong đó kết quả dự đoán được đối chiếu với nhãn thực tế và đo lường bằng các thước đo đánh giá. Các chỉ số như \ac{ac}, \ac{pr}, \ac{re}, \ac{fs} được tính toán theo các công thức trình bày tại \textit{~\ref{eq:accuracy}, ~\ref{eq:precision}, ~\ref{eq:recall}, ~\ref{eq:f1score}}. Những kết quả này là cơ sở quan trọng để so sánh giữa các phương pháp lựa chọn đặc trưng và lựa chọn mô hình phù hợp cho triển khai thực tế.
...
Trong quá trình xây dựng mô hình học máy, việc lựa chọn các siêu tham số (hyperparameters) phù hợp là một yếu tố quan trọng ảnh hưởng đến hiệu năng của mô hình. Để tối ưu hóa các siêu tham số này, phương pháp \textbf{Tìm kiếm theo lưới} (\ac{gs}) kết hợp với \textbf{\ac{cv}} đã được sử dụng trong môi trường phân tán của Apache Spark. Các mô hình phân loại được huấn luyện và so sánh gồm: Decision Tree, Logistic Regression, Linear SVC, Naive Bayes, Random Forest, GBT, XGBoost và MLP; sau đó xếp hạng theo F1-Score để chọn Top-3 mô hình cho các phương pháp ensemble ở giai đoạn 2.

\begin{itemize}
    \item \textbf{\ac{gs}}: Là phương pháp thử nghiệm tất cả các tổ hợp có thể của các siêu tham số được định nghĩa trước. Ví dụ, với một mô hình Random Forest, các tham số có thể bao gồm số lượng cây (\texttt{numTrees}), độ sâu tối đa (\texttt{maxDepth}), và số lượng đặc trưng được chọn tại mỗi nút (\texttt{maxFeatures}). Spark MLlib hỗ trợ đối tượng \texttt{ParamGridBuilder} để xây dựng lưới các tham số.
...
Vai trò                          & Worker huấn luyện (Spark) \& thiết bị suy diễn biên \\
\bottomrule
\end{tabular}
\end{table}
