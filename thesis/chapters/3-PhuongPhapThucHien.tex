...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        \node[lbl] at (10.5,0.95) {RandomSplit 80/20, seed=42};

        \node[box, draw=accBlue,  fill=accBlue!8,   text width=3.6cm] (base) at (17.6,2.5) {\textbf{Base-train} (64\%)\\{\footnotesize fit scaler/PCA/xếp hạng đặc trưng $+$ huấn luyện}};
        ...
        \node[lbl, anchor=east] at (15.4,1.85) {80/20, seed=42};
    \end{tikzpicture}}
    \caption{Sơ đồ phân chia dữ liệu CICIDS2017. Sau tiền xử lý và loại trừ rò rỉ (Thuật toán~\ref{data_preparation_ag}), dữ liệu được chia 80/20 thành train/test (\texttt{seed}=42); tập test được giữ lại, chỉ dùng để công bố kết quả cuối. Trong tập train lại tách 80/20 thành base-train (huấn luyện, và \emph{fit} các phép biến đổi StandardScaler/PCA cùng việc xếp hạng đặc trưng) và validation (chọn Top-3 mô hình thành phần và cặp phương pháp đưa vào kiểm định). Riêng ngưỡng cổng bất thường được chọn trên 10\% lưu lượng benign của tập train (\texttt{seed}=42). Tập test không tham gia bất kỳ khâu lựa chọn nào.}
