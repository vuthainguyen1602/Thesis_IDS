...
\begin{figure}[!t]
    ...
    \begin{tikzpicture}[
        ...
        work/.style  ={draw=accGreen!80!black,  thick, rounded corners, fill=accGreen!8,  text width=5.6cm,  minimum height=2.8cm, inner sep=6pt},
        ...
        \node[work] (w1) at (-3.6,-6.8) {\textbf{Jetson Orin Nano Super \#1}\\[1pt]
            Spark \textbf{Worker} + \textbf{Driver}\\[2pt]
            Executor: cores=4, mem=3\,GB\\
            Phân vùng dữ liệu (partitions)\\
            swap 8\,GB (NVMe)};
        \node[work] (w2) at ( 3.6,-6.8) {\textbf{Jetson Orin Nano Super \#2}\\[1pt]
            Spark \textbf{Worker}\\[2pt]
            Executor: cores=4, mem=3\,GB\\
            Phân vùng dữ liệu (partitions)\\
            swap 8\,GB (NVMe)};
        \node[store] (s1) at (-3.6,-10.6) {Parquet (đồng bộ)};
        \node[store] (s2) at ( 3.6,-10.6) {Parquet (đồng bộ)};
        ...
        \node[lbl, rotate=90] at (-7.3,-3.3) {gom kết quả\\(shuffle/collect)};
