...
Việc huấn luyện mô hình được thực hiện trên một \textbf{cụm Apache Spark Standalone phân tán} gồm một máy chủ và \textbf{hai thiết bị biên NVIDIA Jetson Orin Nano Super Developer Kit}. Cùng một cặp Jetson vừa đóng vai trò worker của cụm Spark trong pha huấn luyện, vừa là thiết bị triển khai trong pha suy luận thời gian thực (Hình~\ref{fig:ids_diagram}), nhờ đó toàn bộ pipeline được kiểm chứng trên đúng phần cứng mục tiêu.

\begin{itemize}
  \item \textbf{Máy chủ (Spark master):} máy tính xách tay MacBook Air, vi xử lý Apple M3, RAM 16 GB, macOS Sonoma 14.3. Máy này đảm nhận điều phối cụm (master), tiền xử lý dữ liệu thô (từ CSV sang Parquet) và chạy hạ tầng Docker (Kafka, PostgreSQL, InfluxDB, Grafana).
  \item \textbf{Hai nút worker:} \textbf{NVIDIA Jetson Orin Nano Super Developer Kit}, hệ điều hành Ubuntu (NVIDIA JetPack/L4T). Cấu hình chi tiết (CPU/GPU/RAM/lưu trữ) được nêu tại Bảng~\ref{tab:hardware}, Chương~3. Jetson~\#1 đồng thời chạy tiến trình \emph{driver} của Spark (driver là tiến trình điều khiển ứng dụng, khác với vai trò \emph{master} điều phối cụm của MacBook).
...
Máy chủ MacBook chỉ đảm nhận vai trò \textit{master} điều phối cụm cùng tiền xử lý và hạ tầng dịch vụ, không tham gia tính toán huấn luyện (không chạy \textit{executor}). Đây là một lựa chọn có chủ đích: số liệu hiệu năng báo cáo trong luận văn phải phản ánh đúng khả năng huấn luyện phân tán ngay trên phần cứng biên Jetson. Nếu đưa MacBook (vi xử lý Apple M3, mạnh hơn đáng kể) vào làm nút tính toán, phần lớn khối lượng công việc sẽ dồn về máy chủ, gây mất cân bằng tải (hiện tượng \textit{straggler}) và làm sai lệch các phép đo thời gian/thông lượng, khiến kết quả không còn phản ánh đúng hiệu năng của cụm biên. Vì vậy, cụm đo đạc được giữ đồng nhất gồm hai nút Jetson, còn máy chủ chỉ được dùng để phát triển và kiểm thử nhanh trong giai đoạn xây dựng pipeline.
...
\vspace{1em}
Bảng~\ref{tab:model_parameters} tóm tắt thiết lập tham số của các mô hình học máy được sử dụng trong nghiên cứu này. Trong đó, \ac{xgb} và MLP được tinh chỉnh bằng Grid Search kết hợp kiểm định chéo (Kịch bản 3). Các mô hình còn lại dùng bộ tham số cố định từ trước, lựa chọn dựa trên các nghiên cứu gần đây và thực nghiệm sơ bộ, nhằm bảo đảm rằng khác biệt quan sát được giữa các phương pháp giảm chiều phản ánh đúng bản thân phương pháp, thay vì mức độ tinh chỉnh riêng từng mô hình.
...
\midrule
\multirow{4}{*}{Logistic Reg.} & maxIter & 200 & Bảo đảm hội tụ trên tập dữ liệu lớn. \\
...
& numTrees & 200 & Bảo đảm độ ổn định kết quả. \\
& maxDepth & 15 & Chiều sâu tối đa của cây trong rừng. \\
...
& subsamplingRate & 0{,}8 & Tỷ lệ lấy mẫu dữ liệu mỗi vòng boosting. \\
\midrule
\multirow{5}{*}{\ac{xgb}}
...
& subsample & 0{,}8 & Tỷ lệ lấy mẫu dữ liệu mỗi cây. \\
& colsample\_bytree & 0{,}8 & Tỷ lệ lấy mẫu đặc trưng mỗi cây. \\
\midrule
\multirow{3}{*}{MLP}
...
\clearpage
% Trang dành riêng cho sơ đồ cụm: căn giữa dọc thay vì dồn lên đầu trang
\vspace*{\fill}
\begin{center}
    \resizebox{\textwidth}{!}{%
    \begin{tikzpicture}[
        ...
        master/.style={draw=gray!65,        thick, rounded corners, fill=gray!8,   text width=12.6cm, minimum height=1.5cm, inner sep=5pt},
        work/.style  ={draw=accGreen!80!black,  thick, rounded corners, fill=accGreen!8,  text width=5.6cm,  minimum height=3.1cm, inner sep=7pt},
        store/.style ={draw=accBlue,         thick, rounded corners, fill=accBlue!5,   text width=5.6cm,  minimum height=1.0cm, inner sep=4pt},
        ...
        \node[work] (w1) at (-3.6,-7.8) {\textbf{Jetson Orin Nano Super \#1}\\[1pt]
            Spark \textbf{Worker} + \textbf{Driver}\\[2pt]
            Executor: cores=4, mem=3\,GB\\
            Phân vùng dữ liệu (partitions)\\
            swap 8\,GB (NVMe)};
        \node[work] (w2) at ( 3.6,-7.8) {\textbf{Jetson Orin Nano Super \#2}\\[1pt]
            Spark \textbf{Worker}\\[2pt]
            Executor: cores=4, mem=3\,GB\\
            Phân vùng dữ liệu (partitions)\\
            swap 8\,GB (NVMe)};
        \node[store] (s1) at (-3.6,-12.2) {Parquet (đồng bộ)};
        \node[store] (s2) at ( 3.6,-12.2) {Parquet (đồng bộ)};
        ...
        \node[lbl, rotate=90] at (-7.3,-3.8) {gom kết quả\\(shuffle/collect)};
        \draw[dn] (w1.south) -- (s1.north);
        \draw[dn] (w2.south) -- (s2.north);
    \end{tikzpicture}}
\captionof{figure}[Cụm huấn luyện phân tán Apache Spark Standalone]{Cụm huấn luyện phân tán Apache Spark Standalone: máy chủ điều phối (master) và hai Jetson worker/executor}
\label{fig:spark_cluster}
\end{center}
\vspace*{\fill}
\clearpage
\subsection{Kịch bản 1: Đánh giá hiệu năng cơ sở (Baseline)}
Trong kịch bản này, toàn bộ tập đặc trưng số (khoảng 78 đặc trưng trước khi loại cổng, chính xác là 77 đặc trưng sau khi loại \texttt{destination\_port}/\texttt{source\_port}) được đưa vào mô hình mà chưa qua bất kỳ bước lựa chọn đặc trưng hay giảm chiều nào. Mục tiêu là thiết lập một mức hiệu năng tham chiếu cho các thử nghiệm tiếp theo.
...
Luận văn thực hiện so sánh 3 chiến lược chính nhằm giảm số lượng đặc trưng nhưng vẫn bảo đảm hiệu năng:
\begin{enumerate}
    \item \textbf{\ac{rf} Feature Importance}: Trích xuất 30 đặc trưng có độ quan trọng cao nhất từ Random Forest.
...
Ngoài ra, việc so sánh trực tiếp giữa hai phương pháp lựa chọn đặc trưng dựa trên độ quan trọng của \ac{rf} và \ac{shap} được trình bày chi tiết trong Bảng~\ref{tab:rf_vs_shap}.
...
\subsection{Kịch bản 3: Tối ưu hóa siêu tham số}
...
Kết quả hiệu năng của phương pháp Tối ưu hóa siêu tham số được trình bày trong Bảng~\ref{tab:exp2_ml_metrics}.
\begin{table}[htbp]
\centering
\caption{Kết quả hiệu năng phân loại kịch bản Tối ưu hóa siêu tham số}
...
Tính giải thích (explainability) là một yêu cầu của hệ thống \ac{ids}, làm cơ sở để tin tưởng và vận hành mô hình trong thực tế. Trong nghiên cứu này, phương pháp \ac{shap} (SHapley Additive exPlanations) được sử dụng để phân tích mức độ quan trọng của từng đặc trưng đối với quyết định phân loại của mô hình \ac{xgb}.
...
\begin{figure}[!t]
    \centering
    \IfFileExists{img/shap_summary_beeswarm.png}{\includegraphics[width=\textwidth]{img/shap_summary_beeswarm.png}}{\fbox{\parbox{0.9\linewidth}{\centering\vspace{1.4cm}\ph{Biểu đồ SHAP beeswarm --- chạy ml\_06\_feature\_selection\_shap.py rồi \texttt{thesis/collect\_figures.sh}}\vspace{1.4cm}}}}
...
\textbf{Ảnh hưởng của cổng đích thay đổi theo họ mô hình.} Con số $\approx$0{,}001 ở trên là đặc tính của họ cây chứ không phải của bộ dữ liệu. Khi lặp lại đúng hai lượt đó trên cùng một phép chia cho cả tám thuật toán (Bảng~\ref{tab:leakage_by_model}, Hình~\ref{fig:leakage_by_model}), mức thổi phồng trải trên gần hai bậc độ lớn: các mô hình cây chỉ được lợi $+0{,}0007$ đến $+0{,}0017$ điểm F1, trong khi \ac{svm} và Logistic Regression được lợi tới $+0{,}0415$ và $+0{,}0395$, tức gấp khoảng 40 lần. Thứ tự này bám theo năng lực biểu diễn của mô hình: các ensemble cây dựng lại được cùng một biên quyết định từ các thống kê hành vi của luồng nên gần như không cần đến cổng, còn mô hình tuyến tính không đủ năng lực làm vậy nên bám vào cổng như một chỉ báo định danh dịch vụ. Hệ quả về phương pháp luận: \emph{không thể suy ra mức rò rỉ của một lớp mô hình từ phép đo trên một mô hình đại diện khác}, và con số nhỏ báo cáo ở Bảng~\ref{tab:leakage_ablation} là con số trung thực cho cấu hình được triển khai, không phải bằng chứng rằng đặc trưng này vô hại nói chung.

\begin{table}[htbp]
\centering
\caption[Ảnh hưởng của \texttt{destination\_port} theo từng thuật toán]{Mức thổi phồng F1 do \texttt{destination\_port} theo từng thuật toán; hai lượt huấn luyện trên cùng một phép chia}
\label{tab:leakage_by_model}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lccc@{}}
\toprule
\textbf{Thuật toán} & \textbf{F1 (loại cổng)} & \textbf{F1 (giữ cổng)} & \textbf{$\Delta$F1} \\ \midrule
\ac{svm} (LinearSVC)   & 0{,}7859 & 0{,}8274 & $+0{,}0415$ \\
Logistic Regression    & 0{,}7986 & 0{,}8381 & $+0{,}0395$ \\
\ac{nb}                & 0{,}2985 & 0{,}3093 & $+0{,}0108$ \\
MLP                    & 0{,}9660 & 0{,}9711 & $+0{,}0052$ \\
\ac{gbt}               & 0{,}9924 & 0{,}9941 & $+0{,}0017$ \\
Decision Tree          & 0{,}9953 & 0{,}9966 & $+0{,}0014$ \\
Random Forest          & 0{,}9954 & 0{,}9964 & $+0{,}0010$ \\
\ac{xgb}               & 0{,}9972 & 0{,}9979 & $+0{,}0007$ \\ \bottomrule
\end{tabular*}
\tabnote{Đây là một lượt chạy độc lập với Bảng~\ref{tab:leakage_ablation}: hàng Random Forest lệch khoảng $1\times10^{-4}$ so với bảng đó do dao động giữa các lần chạy Spark (phân vùng dữ liệu và thứ tự cộng dồn số thực), không phải do khác phương pháp; kết luận về mức chênh $\approx$0{,}001 không đổi.}
\end{table}

\begin{figure}[htbp]
    \centering
    \IfFileExists{img/leakage_ablation_by_model.png}{\includegraphics[width=0.9\textwidth]{img/leakage_ablation_by_model.png}}{\fbox{\parbox{0.8\textwidth}{\centering\vspace{1.2cm}\ph{Biểu đồ ảnh hưởng cổng đích theo mô hình --- chạy ml\_10\_leakage\_ablation.py}\vspace{1.2cm}}}}
    \caption[Mức thổi phồng F1 do \texttt{destination\_port} theo từng thuật toán]{Mức thổi phồng F1 do \texttt{destination\_port} theo từng thuật toán: mô hình tuyến tính phụ thuộc cổng nặng hơn hẳn các ensemble cây}
    \label{fig:leakage_by_model}
\end{figure}

\subsection{Phân tích hiệu năng hệ thống}
Bên cạnh độ chính xác, hiệu năng hệ thống về thời gian và tài nguyên là yếu tố quyết định khả năng triển khai thực tế.
...
\begin{itemize}
    ...
    \item \textbf{Thời gian dự báo}: \ac{xgb} có thời gian dự báo nhanh (khoảng 9 giây cho toàn tập kiểm tra), Random Forest chậm hơn (khoảng 19 giây). Cả hai đều phù hợp cho giám sát theo lô gần thời gian thực.
...
Do các con số F1 giữa những phương pháp/thuật toán hàng đầu rất sát nhau, luận văn không kết luận ``mô hình tốt nhất'' chỉ dựa trên một lần đo. Mô hình đại diện của mỗi phương pháp, cũng như cặp phương pháp dẫn đầu được đưa vào kiểm định, đều được chọn theo F1 trên tập kiểm định (validation, 20\% tách từ tập huấn luyện, \texttt{seed}=42). Tập kiểm tra không tham gia khâu chọn lọc. Hai phương pháp này sau đó được huấn luyện lại trên $N$ lần lấy mẫu lại tập huấn luyện/kiểm tra (mặc định $N{=}6$, ghép cặp trên cùng một phép chia ở mỗi vòng), tính F1 trung bình kèm hai loại khoảng tin cậy 95\%: khoảng tin cậy $t$ thông thường và khoảng tin cậy hiệu chỉnh \emph{Nadeau--Bengio}~\cite{nadeau2003inference}, trong đó phương sai nhân hệ số $(1/N + n_{\text{test}}/n_{\text{train}})$ để bù tương quan giữa các lần lấy mẫu lại chồng lấn. Khoảng Nadeau--Bengio được dùng làm căn cứ kết luận chính. Ý nghĩa thống kê được kiểm bằng permutation test ghép cặp hai phía, liệt kê đầy đủ toàn bộ $2^N = 64$ hoán vị đổi dấu (kiểm định chính xác, không xấp xỉ Monte Carlo), do đó $p$ nhỏ nhất khả dĩ là $2/64 \approx 0{,}031$. Độ lớn hiệu ứng được báo cáo bằng Cohen's $d$ ghép cặp (Bảng~\ref{tab:stat_validity}). Chênh lệch F1 giữa Baseline và \ac{shap} Top-30 có ý nghĩa thống kê ở mức sàn của độ phân giải kiểm định ($p \approx 0{,}031$, $d \approx 1{,}94$), nhưng khác biệt tuyệt đối rất nhỏ ($\approx 0{,}0001$ điểm F1) nên hai phương pháp tương đương về mặt thực dụng. Khi đó, tiêu chí phụ (khả năng giải thích, kích thước, chi phí biên) mới là căn cứ lựa chọn. Với $N{=}6$, kiểm định có lực thống kê thấp. Vì vậy, một giá trị $p$ không có ý nghĩa thống kê tự nó chỉ là bằng chứng yếu cho ``tương đương'', và kết luận được căn cứ đồng thời vào khoảng tin cậy Nadeau--Bengio và Cohen's $d$.

\textbf{Kiểm định tương đương (TOST).} Kiểm định ý nghĩa thông thường chỉ có thể \emph{bác bỏ} giả thuyết ``không khác biệt'', không bao giờ \emph{chứng minh} được hai cấu hình tương đương; đó là lý do lập luận ``$p$ sát sàn nên coi như tương đương'' ở trên vẫn còn yếu. Luận văn do đó bổ sung kiểm định hai phía một đuôi (two one-sided tests, TOST~\cite{schuirmann1987tost}) với biên tương đương \emph{khai báo trước} là $0{,}001$ điểm F1 --- mức chênh lệch còn lại không có ý nghĩa vận hành cho \ac{ids}, và lớn hơn một bậc so với khoảng cách thực tế giữa hai cấu hình. Dùng chính sai số chuẩn đã hiệu chỉnh Nadeau--Bengio ($4{,}58 \times 10^{-5}$) để các lần lấy mẫu lại chồng lấn không làm kết luận đẹp lên giả tạo, kết quả là $p_{\text{TOST}} = 3{,}9 \times 10^{-6}$: hai cấu hình \emph{tương đương} theo nghĩa khẳng định, chứ không chỉ là ``chưa phân biệt được''. Chặt hơn nữa, biên nhỏ nhất mà dữ liệu sáu lần lấy mẫu lại này chứng minh được ở mức $\alpha = 0{,}05$ là $\pm 2{,}30 \times 10^{-4}$ điểm F1 --- tức chênh lệch thật giữa Baseline và \ac{shap} Top-30 chắc chắn nhỏ hơn ngưỡng đó. Tăng $N$ (kèm kiểm định chéo lồng) vẫn là hướng củng cố tiếp theo, nhưng kết luận tương đương không còn phụ thuộc vào một giá trị $p$ nằm sát sàn phân giải.
...
Mô hình đại diện của mỗi phương pháp được đánh giá lại trên một mẫu con của tập kiểm tra (không giao với tập huấn luyện, xem Bảng~\ref{tab:robustness}). Đây chỉ là phép kiểm tra độ ổn định cùng phân phối (in-distribution), không phải kiểm thử dịch chuyển phân phối. Đánh giá tổng quát hóa thực sự nằm ở thí nghiệm liên bộ dữ liệu (Mục~\ref{sec:cross-dataset}).
...
\begin{table}[htbp]
\centering
\caption[Tổng quát hóa liên bộ dữ liệu CICIDS2017 và CSE-CIC-IDS2018]{Tổng quát hóa liên bộ dữ liệu giữa CICIDS2017 và CSE-CIC-IDS2018 (trung bình qua 12 lượt chạy độc lập)}
\label{tab:cross_dataset}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}llccc@{}}
\toprule
\textbf{Huấn luyện} & \textbf{Kiểm tra} & \textbf{Loại} & \textbf{F1} & \textbf{AUC-\ac{pr}} \\ \midrule
CICIDS2017 & CICIDS2017 & cùng bộ & 0{,}9952 & 0{,}9997 \\
CICIDS2017 & CSE-CIC-IDS2018 & khác bộ & 0{,}029--0{,}085 (7 lượt) $\mid$ 0{,}185--0{,}301 (5 lượt) & 0{,}4862 \\
CSE-CIC-IDS2018 & CSE-CIC-IDS2018 & cùng bộ & 0{,}9246 & 0{,}9676 \\
CSE-CIC-IDS2018 & CICIDS2017 & khác bộ & 0{,}0539 $\pm$ 0{,}0022 & 0{,}2072 \\ \bottomrule
\end{tabular*}
\tabnote{Mười hai lượt chạy trên cùng mã nguồn và cùng dữ liệu, dùng bảy hạt giống rừng ngẫu nhiên, trong đó năm hạt giống được lặp lại hai lượt. Chiều 2017$\rightarrow$2018 không ghi trung bình mà ghi trực tiếp hai chế độ, vì mười hai giá trị tách thành hai nhóm rời nhau (0{,}0291--0{,}0843 và 0{,}1853--0{,}3009, không có giá trị nào ở giữa): trung bình 0{,}1321 rơi đúng vào khoảng trống nên không lượt nào cho giá trị đó, và khoảng tin cậy 95\% của trung bình ([0{,}0615; 0{,}2027], Student-$t$, $n=12$) chỉ chứa 2 trong 12 lượt --- ghi con số đó ở đây sẽ gợi ý sai rằng kết quả dao động quanh 0{,}13. Chiều 2018$\rightarrow$2017 đơn đỉnh (khoảng cách lớn nhất giữa hai giá trị liền nhau là 0{,}0036) nên trung bình kèm khoảng tin cậy là cách mô tả hợp lý. Các đại lượng cùng bộ (hệ số biến thiên $\le$ 0{,}1\%) và AUC-\ac{pr} chiều 2017$\rightarrow$2018 (0{,}4532--0{,}5357, hệ số biến thiên 4{,}9\%) thì ổn định.}
\end{table}

\textbf{Diễn giải.} Kết quả cho thấy một tương phản rõ rệt: hiệu năng cùng bộ cao và ổn định ở cả hai chiều (F1 0{,}9952 và 0{,}9246, hệ số biến thiên dưới 0{,}1\% qua 12 lượt chạy), còn hiệu năng khác bộ rơi xuống mức không dùng được. AUC-\ac{pr} cho thấy mức rơi này rõ nhất, vì nó không phụ thuộc ngưỡng quyết định và ổn định giữa các lượt chạy: từ 0{,}9997 xuống 0{,}4862 ở chiều 2017$\rightarrow$2018 và từ 0{,}9676 xuống 0{,}2072 ở chiều ngược lại, tức mất một nửa đến bốn phần năm khả năng xếp hạng luồng độc hại. F1 khác bộ nằm ở hai chế độ 0{,}0291--0{,}0843 và 0{,}1853--0{,}3009 ở chiều 2017$\rightarrow$2018, còn chiều ngược lại tập trung quanh 0{,}0539 (0{,}0484--0{,}0587); mức thấp này đến từ recall rất nhỏ (trung bình 0{,}075 và 0{,}030) trong khi precision vẫn còn khá (0{,}764 và 0{,}270), tức mô hình gần như không nhận ra tấn công của bộ dữ liệu kia, chứ không tạo ra cảnh báo sai tràn lan. Nguyên nhân là dịch chuyển phân phối đặc trưng giữa hai testbed: khác phiên bản CICFlowMeter (thang đo và cách tính một số đặc trưng thay đổi), khác hạ tầng mạng và cấu hình dịch vụ, khác tổ hợp loại tấn công (HOIC/LOIC-HTTP của 2018 không có trong 2017). Sự sụt giảm này nhất quán với các nghiên cứu cross-dataset trên IDS dựa trên đặc trưng luồng~\cite{dhooge2020interdataset,cantone2024crossdataset,sarhan2022explainable}, và củng cố luận điểm trung tâm của luận văn: con số trong-miền gần hoàn hảo, kể cả khi đã loại rò rỉ, không bảo đảm khả năng triển khai sang môi trường khác. Đánh giá liên bộ dữ liệu phải là một phần bắt buộc của quy trình kiểm định IDS~\cite{goldschmidt2025datasets}, và các hướng thích ứng miền (chuẩn hóa theo miền, huấn luyện liên bộ, cập nhật theo drift) là điều kiện tiên quyết cho triển khai thực tế.

\textbf{F1 khác bộ không phải một con số tái lập được.} Mười hai lượt chạy ở Bảng~\ref{tab:cross_dataset} dùng cùng mã nguồn, cùng tệp parquet và cùng danh sách đặc trưng; chỉ hạt giống của rừng ngẫu nhiên thay đổi, và năm hạt giống được chạy hai lượt để đo riêng phần dao động \emph{trong cùng một cấu hình}. Kết quả ở chiều 2017$\rightarrow$2018 không phải nhiễu quanh một giá trị trung tâm mà là \emph{hai chế độ rời nhau}: bảy lượt cho F1 0{,}0291--0{,}0843 với recall 0{,}0148--0{,}0447, năm lượt cho F1 0{,}1853--0{,}3009 với recall 0{,}1036--0{,}1778, và không một lượt nào rơi vào khoảng giữa 0{,}0843 và 0{,}1853.

Chế độ của một lượt do quá trình huấn luyện quyết định, \emph{không phải} hạt giống. Trong năm hạt giống được lặp, ba hạt giống cho hai lượt cùng chế độ và lệch nhau rất ít (seed 42: 0{,}3009 và 0{,}2936; seed 23: 0{,}0411 và 0{,}0434; seed 31: 0{,}0291 và 0{,}0303), nhưng \emph{hai hạt giống đổi hẳn chế độ giữa hai lượt}: seed 5 cho 0{,}0495 rồi 0{,}1854, seed 11 cho 0{,}0843 rồi 0{,}2606. Nghĩa là cùng một hạt giống, cùng một tệp dữ liệu vẫn có thể cho hai kết quả cách nhau hơn ba lần (3{,}1 lần ở seed 11 và 3{,}8 lần ở seed 5); nguồn dao động nằm ở chính quá trình huấn luyện phân tán, nơi thứ tự gộp các phân vùng khi Spark tính biểu đồ tần suất chia nút không cố định giữa các lượt, nên một vài ngưỡng chia nút được chọn khác đi.

Chính vị trí ngưỡng quyết định 0{,}5 khuếch đại sai lệch nhỏ đó thành hai chế độ. Xác suất dự đoán của gần như toàn bộ luồng tấn công miền đích nằm \emph{sát ngay} ngưỡng này, mà các luồng đó lại rất giống nhau (cùng vài họ tấn công, cùng bộ công cụ sinh ra), nên chúng không vượt ngưỡng một cách rời rạc: cả một cụm lớn cùng lật qua hoặc cùng tụt xuống dưới ngưỡng, đưa recall nhảy thẳng từ $\approx$0{,}03 lên $\approx$0{,}15 mà không đi qua các giá trị trung gian. AUC-\ac{pr} xác nhận cách giải thích này: không dùng ngưỡng, nó \emph{như nhau ở cả hai chế độ} (0{,}4590--0{,}5163 ở chế độ thấp so với 0{,}4532--0{,}5357 ở chế độ cao, hai khoảng trùm lên nhau hoàn toàn). Khả năng xếp hạng của mô hình không đổi giữa các lượt; chỉ vị trí của ngưỡng 0{,}5 so với cụm luồng đó là đổi.

Hai hệ quả về phương pháp đánh giá. Thứ nhất, trung bình không phải một cách mô tả tốt cho phân phối hai chế độ: con số 0{,}1321 ở Bảng~\ref{tab:cross_dataset} nằm đúng vào khoảng trống giữa hai chế độ, tức \emph{không lượt chạy nào} cho giá trị ấy. Thứ hai, một con số F1 khác bộ từ một lượt chạy duy nhất không mang thông tin về mô hình mà chủ yếu cho biết lượt đó rơi vào chế độ nào --- mẫu 0{,}0423 từng được ghi trong các bản thảo trước của luận văn và của bài báo, lưu tại \texttt{cross\_dataset\_results.published.bak}, là một mẫu thuộc chế độ thấp. Vì vậy luận văn báo cáo cả phân phối (số lượt mỗi chế độ và khoảng giá trị) và dùng AUC-\ac{pr} làm đại lượng chính khi so sánh khác bộ. Kết luận định tính không đổi: kể cả ở chế độ cao, F1 khác bộ 0{,}30 và AUC-\ac{pr} 0{,}49 vẫn cách rất xa mức cùng bộ 0{,}99 và 0{,}9997, và khoảng cách đó không cấu hình nào lấp được.

\textbf{Thử hai kỹ thuật thích ứng miền không giám sát.} Câu hỏi tự nhiên tiếp theo là phần sụt giảm đó có phải chỉ do lệch thang đo giữa hai testbed hay không --- nếu đúng vậy thì một phép căn chỉnh tuyến tính là đủ để khắc phục. Luận văn kiểm chứng bằng hai kỹ thuật kinh điển, cả hai đều \emph{không dùng bất kỳ nhãn nào của miền đích} và \emph{không huấn luyện lại bộ phân loại}, tức đúng tình huống của một hệ đã triển khai gặp mạng mới: (i)~\emph{chuẩn hóa theo miền} --- giữ nguyên rừng cây, chỉ fit lại \texttt{StandardScaler} trên phần đặc trưng (không nhãn) của tập huấn luyện miền đích; (ii)~\emph{CORAL}~\cite{sun2016coral} --- làm trắng miền đích bằng hiệp phương sai của chính nó rồi ``tô'' lại bằng hiệp phương sai miền nguồn, tức căn khớp cả moment bậc hai. Kết quả ở Bảng~\ref{tab:cross_adapt}.

\begin{table}[htbp]
\centering
\caption[Thích ứng miền không giám sát trên tác vụ liên bộ dữ liệu]{Hai kỹ thuật thích ứng miền không giám sát trên tác vụ liên bộ dữ liệu (bộ phân loại giữ nguyên, không dùng nhãn miền đích)}
\label{tab:cross_adapt}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}llcc@{}}
\toprule
\textbf{Chiều} & \textbf{Cấu hình} & \textbf{F1} & \textbf{AUC-\ac{pr}} \\ \midrule
\multirow{3}{*}{2017 $\rightarrow$ 2018} & Không thích ứng & 0{,}2978 & 0{,}5261 \\
 & Chuẩn hóa theo miền & 0{,}0000 & 0{,}3169 \\
 & CORAL & 0{,}0000 & 0{,}2015 \\ \midrule
\multirow{3}{*}{2018 $\rightarrow$ 2017} & Không thích ứng & 0{,}0575 & 0{,}1791 \\
 & Chuẩn hóa theo miền & 0{,}0000 & 0{,}1068 \\
 & CORAL & 0{,}0000 & \textbf{0{,}2496} \\ \bottomrule
\end{tabular*}
\tabnote{Ba dòng của mỗi chiều đến từ \emph{cùng một lượt chạy} nên so sánh được với nhau. Dữ liệu thô của bảng này là \texttt{cross\_dataset\_adaptation\_high\_regime.csv} (hạt giống 42). Kết quả F1 về 0 được kiểm chứng trên \emph{ba} lượt chạy độc lập có mức cơ sở 0{,}2939--0{,}3000, tức đều thuộc chế độ cao của phân phối đã mô tả ở trên; mức cơ sở của Bảng~\ref{tab:label_budget} (0{,}2939) cũng thuộc chế độ này nên hai bảng nhất quán với nhau. Trường hợp mức cơ sở thuộc chế độ thấp chưa đo được, và không thể chỉ định trước vì chế độ không do hạt giống quyết định --- đây là hạn chế của thí nghiệm thích ứng, không phải của kết luận về sự bất ổn. Ánh xạ CORAL được kiểm chứng giải tích: sau khi ánh xạ, trung bình và hiệp phương sai của miền đích khớp miền nguồn với sai số tương đối $4\times10^{-12}$.}
\end{table}

Cả hai kỹ thuật đều \emph{không} khôi phục được F1: sau khi căn chỉnh, mô hình gần như ngừng báo tấn công nên F1 về 0. Điều này bác bỏ giả thuyết ``chỉ lệch thang đo'': nếu dịch chuyển nằm ở moment bậc một và bậc hai thì hai phép căn chỉnh này đã đủ, nhưng thực tế phần khác biệt nằm ở \emph{phân phối có điều kiện} --- cái gì làm một luồng trở thành độc hại --- cùng với việc hai testbed dùng bộ công cụ tấn công không trùng nhau. Tuy vậy có một chi tiết đáng chú ý: ở chiều 2018$\rightarrow$2017, CORAL \emph{cải thiện} AUC-\ac{pr} từ 0{,}1791 lên 0{,}2496 trong khi F1 vẫn bằng 0. Nghĩa là phép căn chỉnh có làm \emph{thứ tự xếp hạng} tốt lên, nhưng ngưỡng quyết định 0{,}5 học từ miền nguồn không còn hiệu lực trong không gian đặc trưng đã đổi. Bài học rút ra cho triển khai thực tế: thích ứng miền cho \ac{ids} không thể dừng ở căn chỉnh đặc trưng mà phải kèm \emph{hiệu chỉnh lại ngưỡng} trên miền đích.

\textbf{Cái giá của việc gán nhãn: 1\% dữ liệu miền đích.} Nếu không kỹ thuật không giám sát nào hiệu quả, câu hỏi thực tế tiếp theo là \emph{phải gán nhãn bao nhiêu} lưu lượng của mạng mới để có bộ phát hiện dùng được. Luận văn đo trực tiếp: lấy ngẫu nhiên 1\% tập huấn luyện của miền đích \emph{kèm nhãn} (giữ tỷ lệ tấn công tự nhiên, không phải mẫu cân bằng 50/50, vì kịch bản là gán nhãn những gì đi qua mạng), huấn luyện lại mô hình từ đầu, rồi đánh giá trên tập kiểm tra miền đích chưa hề đụng tới. Hai cấu hình được so sánh: (i)~\emph{gộp} dữ liệu nguồn với 1\% nhãn đích và (ii)~\emph{chỉ dùng} 1\% nhãn đích, bỏ hẳn dữ liệu nguồn --- cấu hình thứ hai đóng vai trò đối chứng để trả lời câu hỏi dữ liệu nguồn có đóng góp gì không. Kết quả ở Bảng~\ref{tab:label_budget}.

\begin{table}[htbp]
\centering
\caption[Ngân sách nhãn miền đích: 1\% đủ để phục hồi hiệu năng]{Ngân sách nhãn miền đích: huấn luyện lại với 1\% dữ liệu có nhãn của miền đích}
\label{tab:label_budget}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}llrcc@{}}
\toprule
\textbf{Chiều} & \textbf{Cấu hình} & \textbf{Nhãn đích} & \textbf{F1} & \textbf{Recall} \\ \midrule
\multirow{3}{*}{2017 $\rightarrow$ 2018}
 & Không thích ứng & 0 & 0{,}2939 & 0{,}1749 \\
 & Nguồn $+$ 1\% nhãn đích & 19.276 & 0{,}8034 & 0{,}6750 \\
 & \textbf{Chỉ 1\% nhãn đích} & 19.276 & \textbf{0{,}9223} & 0{,}9198 \\ \midrule
\multirow{3}{*}{2018 $\rightarrow$ 2017}
 & Không thích ứng & 0 & 0{,}0574 & 0{,}0322 \\
 & Nguồn $+$ 1\% nhãn đích & 18.011 & 0{,}9607 & 0{,}9853 \\
 & \textbf{Chỉ 1\% nhãn đích} & 18.011 & \textbf{0{,}9897} & 0{,}9849 \\ \bottomrule
\end{tabular*}
\tabnote{1\% được lấy từ \emph{tập huấn luyện} của miền đích, không bao giờ từ tập kiểm tra. Dòng ``không thích ứng'' lấy từ chính lượt chạy này để ba dòng so sánh được với nhau; mức cơ sở 0{,}2939 vì vậy là một lượt thuộc \emph{chế độ cao} của phân phối F1 khác bộ nói trên, cùng chế độ với lượt dùng ở Bảng~\ref{tab:cross_adapt} (0{,}2978). Kết luận rút ra dưới đây không phụ thuộc vào mức cơ sở cụ thể: kể cả khi so với mẫu cao nhất quan sát được (0{,}3009), mức tăng do 1\% nhãn vẫn là hơn một bậc độ lớn ở cả hai chiều.}
\end{table}

Hai điều rút ra, điều thứ hai là bất ngờ. Thứ nhất, \textbf{khoảng 19 nghìn luồng có nhãn là đủ}: F1 chiều 2017$\rightarrow$2018 tăng từ 0{,}29 lên 0{,}92 và chiều ngược lại từ 0{,}06 lên 0{,}99. Nghĩa là mạng mới \emph{không} khó phát hiện --- cái không chuyển được là kiến thức của mô hình cũ. Thứ hai, \textbf{dữ liệu miền nguồn không những vô ích mà còn có hại}: chỉ dùng 19 nghìn dòng nhãn đích cho F1 0{,}9223, trong khi gộp thêm 1,79 triệu dòng có nhãn của miền nguồn lại kéo xuống 0{,}8034 (recall rơi từ 0{,}92 xuống 0{,}68); chiều ngược lại cũng cùng xu hướng (0{,}9897 so với 0{,}9607). Khối lượng áp đảo của dữ liệu nguồn kéo mô hình về những mẫu hình đặc thù của testbed cũ, lấn át tín hiệu từ phần dữ liệu đích ít ỏi nhưng đúng phân phối.

Khuyến nghị triển khai rút ra từ đây rất cụ thể: khi đưa hệ thống sang một mạng mới, \emph{đừng} tái sử dụng mô hình cũ hay gộp dữ liệu cũ vào; hãy gán nhãn một lượng nhỏ lưu lượng tại chỗ (cỡ 1\%, tương đương vài giờ công của một phân tích viên) rồi huấn luyện lại từ đầu. Đây cũng là lời giải thích vì sao các kỹ thuật thích ứng không giám sát ở trên thất bại: khoảng cách giữa hai miền không nằm ở phép biến đổi tuyến tính của không gian đặc trưng mà ở chính ranh giới quyết định, và ranh giới đó chỉ học được từ nhãn của miền đích.

\subsection{Đánh giá đa lớp theo loại tấn công}
\label{sec:multiclass}
Phân loại nhị phân (Benign/Attack) trên CICIDS2017 vốn dễ tách. Để bộc lộ điểm yếu thực sự, luận văn bổ sung đánh giá đa lớp theo từng loại tấn công gốc (precision/recall/F1 từng lớp, macro-F1, weighted-F1 và ma trận nhầm lẫn), gồm lớp Benign và 14 loại tấn công của CICIDS2017. Kết quả cho thấy tương phản rất rõ giữa hai chỉ số tổng hợp: weighted-F1 $\approx 0{,}998$ (bị chi phối bởi các lớp đa số) nhưng macro-F1 chỉ $\approx 0{,}806$. Đây là bằng chứng cho thấy độ chính xác nhị phân gần hoàn hảo đã che giấu hiệu năng phát hiện kém ở các lớp hiếm. Cụ thể, các lớp đa số (Benign, DoS Hulk, DDoS, PortScan) đạt F1 rất cao ($\geq 0{,}97$), trong khi các lớp cực hiếm suy giảm nghiêm trọng: Web Attack~-- XSS recall chỉ $0{,}027$, Web Attack~-- SQL Injection F1~$=0$ (chỉ 6 mẫu), Bot recall $0{,}46$. Đây là nơi khác biệt giữa các phương pháp lựa chọn đặc trưng thể hiện rõ nhất và cũng là hướng cần cải thiện (xử lý mất cân bằng, tăng cường dữ liệu lớp hiếm). Kết quả chi tiết theo từng lớp được trình bày trong Bảng~\ref{tab:per_class}, và ma trận nhầm lẫn đa lớp tương ứng được minh họa ở Hình~\ref{fig:confusion_matrix}.
...
\begin{figure}[!t]
    \centering
    \IfFileExists{img/confusion_matrix.png}{\includegraphics[width=0.95\textwidth]{img/confusion_matrix.png}}{\fbox{\parbox{0.66\textwidth}{\centering\vspace{1.4cm}\ph{Ma trận nhầm lẫn đa lớp --- chạy ml\_09\_multiclass\_eval.py}\vspace{1.4cm}}}}
    \caption{Ma trận nhầm lẫn đa lớp theo loại tấn công (chuẩn hóa theo hàng)}
    \label{fig:confusion_matrix}
\end{figure}

\textbf{Phân tích hướng nhầm lẫn giữa các lớp hiếm.} Đọc ma trận nhầm lẫn theo \emph{đích đến} của các dự đoán sai cho thấy các lớp hiếm suy giảm theo hai kiểu khác nhau. Kiểu thứ nhất là \emph{nhầm loại nhưng vẫn bị phát hiện}: 81/111 mẫu Web Attack~--~XSS bị gán thành Web Attack~--~Brute Force, tức ở bài toán nhị phân triển khai trên biên, các luồng này \emph{vẫn bị gắn cờ tấn công} (76\% XSS vẫn bị chặn), chỉ sai ở khâu định danh loại. Kiểu thứ hai nguy hiểm hơn là \emph{lọt hẳn thành Benign}: toàn bộ 6/6 mẫu SQL Injection, 158/290 mẫu Bot (54\%) và 84/311 mẫu Web Brute Force bị dự đoán là lưu lượng bình thường. Ba cơ chế giải thích hiện tượng này: (i)~\emph{mất cân bằng cực đoan} (6--311 mẫu so với 380.119 mẫu Benign) khiến các lớp hiếm gần như không có vùng quyết định riêng; (ii)~việc \emph{loại \texttt{destination\_port}} ảnh hưởng nặng nhất đến nhóm tấn công web: khi mất ``lối tắt'' cổng 80, các luồng HTTP tấn công (request/response ngắn) có đặc trưng hành vi rất gần lưu lượng duyệt web thường và gần lẫn nhau, do đó giá trị recall 0{,}027 của XSS là con số trung thực thay cho con số đẹp nhờ rò rỉ; (iii)~\emph{Bot lẫn Benign do bản chất}: các luồng beaconing C\&C nhỏ, đều đặn, giống lưu lượng nền khi không còn định danh cổng/dịch vụ. Nhận diện này dẫn thẳng tới các hướng phát triển: xử lý mất cân bằng có chủ đích (tăng cường/oversampling lớp hiếm), bổ sung đặc trưng chuỗi thời gian cho hành vi beaconing, và kiểm chứng liên bộ dữ liệu (Mục~\ref{sec:cross-dataset}).

\clearpage
\section{Thực nghiệm và đánh giá hiệu năng mô hình trên Jetson Orin Nano Super}

Trong phần này, luận văn đề xuất xây dựng một hệ thống phát hiện xâm nhập mạng \ac{ids} phân tán triển khai trên một cụm \textbf{hai thiết bị biên NVIDIA Jetson Orin Nano Super}~\cite{nvidia2019jetson} nhằm đánh giá khả năng hoạt động trong môi trường tài nguyên hạn chế. Đây là hướng tiếp cận điện toán biên đưa phân tích về gần nguồn dữ liệu để giảm băng thông và thời gian phản hồi \cite{shi2016edge,hamdan2020edge}, trong khi các nghiên cứu IDS biên trước đây chủ yếu đánh giá suy luận đơn nút \cite{sforzin2016rpids}. Hệ thống được thiết kế theo kiến trúc xử lý dữ liệu thời gian thực hai tầng: một cổng phát hiện bất thường (anomaly gate) bằng autoencoder nhẹ lọc bớt lưu lượng bình thường trên Jetson~\#1, và một bộ phân loại PySpark \texttt{PipelineModel} chỉ xử lý các luồng nghi vấn trên Jetson~\#2. Hạ tầng trung tâm (Apache Kafka cho truyền tải thông điệp, PostgreSQL để quản lý dữ liệu, InfluxDB để lưu chỉ số hiệu năng theo thời gian, Grafana để trực quan hóa và Mailtrap để kiểm thử cảnh báo qua email) chạy trên một máy chủ điều phối.
...
\begin{itemize}
    ...
    \item \textbf{Apache Kafka}~\cite{kreps2011kafka} -- Là nền tảng truyền tải thông điệp phân tán (distributed event streaming platform), được sử dụng để tiếp nhận và phân phối dữ liệu luồng theo cơ chế publish-subscribe. Trong hệ thống, Kafka đóng vai trò trung gian giữa nguồn dữ liệu (CICIDS2017 được mô phỏng bằng Python script) và thiết bị Jetson Orin Nano Super, bảo đảm khả năng truyền tải bất đồng bộ, chịu lỗi và mở rộng linh hoạt.
...
Các công nghệ trên bao phủ đủ bốn nhu cầu vận hành của hệ thống: truyền tải luồng, lưu trữ, giám sát hiệu năng và cảnh báo.
...
\begin{enumerate}
    ...
    Jetson~\#1 đảm nhận vai trò \texttt{anomaly\_gate}: một autoencoder nhẹ (huấn luyện chỉ trên lưu lượng bình thường) tính sai số tái tạo (reconstruction error) cho từng luồng. Các luồng có sai số dưới ngưỡng được coi là bình thường và bị lọc bỏ. Chỉ các luồng nghi vấn mới được chuyển tiếp sang topic \texttt{ids-suspicious-flow}. Cơ chế này giảm đáng kể khối lượng phải đưa vào suy luận Spark.
    ...
    Mỗi nút ghi nhận thời gian dự đoán/mẫu, mức sử dụng CPU/RAM/nhiệt độ và thông lượng (gắn nhãn theo \texttt{EDGE\_NODE\_ID}) và lưu các chỉ số này vào InfluxDB.
...
Kiến trúc này tách các thành phần xử lý thành những mô-đun độc lập, phù hợp với triển khai phân tán trên thiết bị biên tài nguyên hạn chế như Jetson Orin Nano Super.

\textbf{Đặc tả cổng bất thường (operating point).} Cổng autoencoder là một bộ lọc điều chỉnh được chứ không phải một điểm vận hành cố định: nâng ngưỡng giúp giảm tải (offload) nhiều lưu lượng bình thường hơn khỏi Spark (tăng tỷ lệ bỏ qua tại cổng, gate-skip) nhưng có thể giảm recall trên lớp tấn công. Ngưỡng của cổng được chọn theo phân vị (quantile) của sai số tái tạo tính trên một tập kiểm định benign gồm 10\% tách từ phần lưu lượng bình thường của tập huấn luyện (\texttt{seed}=42). Tập kiểm tra không tham gia việc chọn ngưỡng, tránh rò rỉ kiểu ``ngưỡng oracle''. Bảng~\ref{tab:gate_oc} trình bày đường đánh đổi giữa recall và mức giảm tải (recall--offload) khi quét các mức quantile trên tập kiểm định này. Điểm vận hành được chọn theo ngân sách độ trễ/độ chính xác của triển khai. FPR trong bảng là tỷ lệ luồng benign bị cổng chuyển nhầm sang bộ phân loại, không phải FPR của toàn hệ thống. Cần lưu ý phạm vi tính toán: tỷ lệ bỏ qua trong Bảng~\ref{tab:gate_oc} được tính trên tập kiểm tra đã loại trùng lặp ở mức đặc trưng (380.119 benign / 67.156 attack, tức khoảng 85\% benign), còn tỷ lệ bỏ qua thực đo khi triển khai (95{,}8\% ở chế độ A và 97{,}9\% ở chế độ C, Bảng~\ref{tab:edge_modes}) được tính trên luồng phát lại tuần tự từ các tệp CSV gốc của CICIDS2017, vốn còn nguyên các luồng trùng lặp (phần lớn là benign) nên tỷ lệ benign trong cửa sổ đo cao hơn. Cùng một ngưỡng nhưng khác thành phần lưu lượng thì cho tỷ lệ bỏ qua khác nhau, vì vậy con số thực đo không phải là một điểm nằm trên đường đánh đổi của bảng này và hai giá trị không so sánh trực tiếp được.
...
\begin{figure}[htbp]
    \centering
    \IfFileExists{img/pipeline_terminal_jetson1.png}{\includegraphics[width=0.85\textwidth]{img/pipeline_terminal_jetson1.png}}{\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}\ph{Ảnh terminal node cổng (jetson-nano-1)}\vspace{1.2cm}}}}\\[2pt]
    {\footnotesize (a) Jetson \#1 (\texttt{jetson-nano-1}): cổng phát hiện bất thường, giám sát tài nguyên theo node}\\[8pt]
    \IfFileExists{img/pipeline_terminal_jetson2.png}{\includegraphics[width=0.85\textwidth]{img/pipeline_terminal_jetson2.png}}{\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}\ph{Ảnh terminal node phân loại (jetson-nano-2)}\vspace{1.2cm}}}}\\[2pt]
    ...
    \caption[Giao diện terminal của pipeline IDS trên hai nút Jetson]{Giao diện terminal của pipeline IDS trên hai nút Jetson: (a) cổng lọc bất thường, (b) bộ phân loại PySpark}
    \label{fig:pipeline_terminal}
\end{figure}

\begin{figure}[htbp]
    \centering
    \IfFileExists{img/grafana_dashboard.png}{\includegraphics[width=0.9\textwidth]{img/grafana_dashboard.png}}{\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}\ph{Dashboard Grafana hiệu năng --- chụp lại từ cụm Jetson}\vspace{1.2cm}}}}
...
\begin{figure}[htbp]
    \centering
    \IfFileExists{img/grafana_attacks.png}{\includegraphics[width=0.9\textwidth]{img/grafana_attacks.png}}{\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}\ph{Dashboard Grafana chi tiết tấn công --- chụp lại từ cụm Jetson}\vspace{1.2cm}}}}
...
\begin{figure}[htbp]
    \centering
    \IfFileExists{img/grafana_peak_load.png}{\includegraphics[width=0.9\textwidth]{img/grafana_peak_load.png}}{\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}\ph{Dashboard Grafana tại thời điểm tải đỉnh --- chụp lại từ cụm Jetson}\vspace{1.2cm}}}}
...
\begin{figure}[htbp]
    \centering
    \IfFileExists{img/mailtrap_alert.png}{\includegraphics[width=0.9\textwidth]{img/mailtrap_alert.png}}{\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}\ph{Email cảnh báo Mailtrap --- chụp lại từ hệ chạy trên Jetson}\vspace{1.2cm}}}}
...
\begin{figure}[htbp]
    \centering
    \IfFileExists{img/slack_alert.png}{\includegraphics[width=0.9\textwidth]{img/slack_alert.png}}{\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}\ph{Thông báo Slack --- chụp lại từ hệ chạy trên Jetson}\vspace{1.2cm}}}}
...
Về tiêu chí nhận biết ràng buộc biên (edge-aware), ba mô hình ứng viên đánh đổi khác nhau: Decision Tree có kích thước nhỏ nhất (0{,}046\,MB, nhỏ hơn Random Forest khoảng 96 lần) và huấn luyện nhanh hơn khoảng 28 lần, trong khi Random Forest đạt F1 cao nhất. Riêng F1/Accuracy chưa đủ để kết luận: quyết định cuối cùng dựa trên số đo vận hành thực tế trên thiết bị (thông lượng, độ trễ, CPU, RAM) ở mục tiếp theo. (Các mô hình ứng viên biên được huấn luyện trên mẫu con phân tầng khi xuất để vừa bộ nhớ một nút, nên F1 thấp hơn chút ít so với kết quả huấn luyện phân tán đầy đủ ở Bảng~\ref{tab:summary_comparison}. Độ chính xác chính thức lấy từ pha huấn luyện đầy đủ.)
...
Luận văn triển khai mô hình dưới dạng \texttt{PipelineModel} của Spark để đồng nhất với pha huấn luyện phân tán: cùng một stack vừa huấn luyện trên cụm vừa suy luận trên thiết bị. Để định lượng chi phí đánh đổi của sự đồng nhất này, luận văn đo lại cùng một mô hình Random Forest (cùng siêu tham số) trên cùng tập đặc trưng SHAP đã chọn để triển khai nhưng qua các engine nhẹ hơn ở chế độ một nút: scikit-learn (CPU, không JVM) và ONNX Runtime (CPU execution provider, xem Bảng~\ref{tab:engines_thesis}, sinh từ \texttt{benchmark\_engines.py}). Cả ba engine được đo trên nút phân loại với batch 10. Năng lượng báo cáo là phần hoạt động, đã trừ nền idle. Vì cùng thuật toán và đặc trưng nên độ chính xác tương đương. Phép so sánh vì vậy tập trung vào chi phí suy luận. (Cả ba dòng của Bảng~\ref{tab:engines_thesis} được đo lại trong cùng một phiên trên cùng trạng thái bo mạch để so sánh công bằng, nên dòng PySpark chênh lệch nhỏ so với Bảng~\ref{tab:edge_benchmark} đo ở phiên trước: 22{,}6 so với 26{,}8 luồng/giây, do khác phiên đo và xung CPU.) Kết quả đo (Bảng~\ref{tab:engines_thesis}) cho thấy stack JVM/Spark chịu chi phí suy luận cao hơn đáng kể so với scikit-learn trên bo mạch ARM 8\,GB: thông lượng thấp hơn khoảng 9 lần, độ trễ p95 cao hơn khoảng 11 lần, tỷ lệ RAM gần gấp đôi và năng lượng mỗi suy luận cao hơn khoảng 25 lần. Biên dịch cùng mô hình rừng ngẫu nhiên sang ONNX Runtime còn nới rộng khoảng cách này đáng kể: $\approx$7\,900 luồng/giây với p95 1{,}1\,ms, tức gấp $\approx$348 lần thông lượng của Spark đã triển khai, ở mức năng lượng chỉ $\approx$2\% so với scikit-learn. Khoảng cách đó là cơ sở cho hướng phát triển xuất mô hình sang ONNX/TensorRT để suy luận trong khi vẫn giữ Spark cho huấn luyện (việc tích hợp phiên ONNX vào pipeline streaming và khai thác GPU qua TensorRT thuộc hướng phát triển).
...
Ngoài việc so sánh các mô hình trên một nút, luận văn đánh giá ba chế độ triển khai phân tán trên cụm hai Jetson Orin Nano Super (Bảng~\ref{tab:edge_modes}). Chỉ số chính gồm thông lượng, độ trễ p95 (ms), tỷ lệ bỏ qua tại cổng (gate-skip: \% lưu lượng bình thường được cổng lọc khỏi suy luận Spark) và F1 trên lớp tấn công. \emph{Thông lượng} được định nghĩa là số phán quyết cuối cùng ghi vào PostgreSQL trên giây (một luồng nhận phán quyết hoặc tại cổng (nếu bị bỏ qua là benign) hoặc tại bộ phân loại), không cộng dồn đầu ra của hai tầng để tránh đếm trùng luồng được chuyển tiếp ở chế độ pipeline split.
...
\begin{figure}[!t]
    \centering
    \IfFileExists{img/edge_modes.png}{\includegraphics[width=0.95\textwidth]{img/edge_modes.png}}{\fbox{\parbox{0.8\textwidth}{\centering\vspace{1.2cm}\ph{Biểu đồ so sánh chế độ triển khai --- chạy plot\_edge\_modes.py}\vspace{1.2cm}}}}
...
Kết quả thực đo (Hình~\ref{fig:edge_modes}) cho thấy: chế độ \emph{pipeline split} tách cổng khỏi bộ phân loại Spark giúp thông lượng phán quyết tăng từ 2{,}6 lên 62{,}5 luồng/giây (gấp $\approx$24 lần) với p95 gần như không đổi, nhờ cổng lọc 95{,}8\% lưu lượng bình thường và không còn bị Spark chặn vòng xử lý (tỷ lệ này tính trên luồng phát lại, không so sánh trực tiếp với Bảng~\ref{tab:gate_oc}). \emph{Mở rộng theo chiều ngang} chỉ tăng xấp xỉ gấp đôi một nút (5{,}9 so với 2{,}6) vì mỗi nút vẫn chạy trọn bộ phân loại Spark trong tiến trình. \emph{Spark cluster} (cổng + bộ phân loại nối cụm Spark standalone) đạt thông lượng trung bình cao nhất (90{,}0) nhưng dao động lớn giữa các lần chạy ($\pm$26) và phụ thuộc thêm nút chủ (master) trên máy điều phối, nên pipeline split vẫn là khuyến nghị mặc định. Năng lượng mỗi suy luận đo bằng \texttt{tegrastats} (nền idle 30\,giây trước mỗi cửa sổ tải, chế độ hai nút cộng công suất cả hai bo mạch): ở mức tải 100 luồng/giây, phần công suất hoạt động (đã trừ idle) nằm dưới ngưỡng nhiễu lấy mẫu ($<$0{,}15\,W mọi nút), do đó bảng báo cáo năng lượng toàn bo mạch trên mỗi phán quyết, tức thước đo mức dàn trải công suất bo mạch theo thông lượng ở cấp triển khai (không phải chi phí riêng của mô hình): các chế độ dùng cổng (A, C) dàn trải công suất hai bo mạch trên số phán quyết lớn gấp 24--35 lần, giảm năng lượng mỗi phán quyết từ $\approx$2{,}5\,J xuống 0{,}12--0{,}18\,J.
...
\begin{itemize}
    \item \textbf{Random Forest} thắng ở cả ba chỉ số vận hành quan trọng nhất đối với phát hiện xâm nhập thời gian thực: thông lượng cao nhất (26{,}8 mẫu/giây), độ trễ p95 thấp nhất (397\,ms) và CPU thấp nhất (35{,}2\%), nhờ Spark song song hóa tốt trên nhiều cây. Cái giá phải trả là RAM cao nhất (27{,}5\%) và mô hình lớn nhất ($\approx$4{,}4\,MB).
...
\begin{enumerate}
    ...
    \item \textbf{Chất lượng phát hiện}: cao nhất trong ba mô hình ứng viên ở cả hai phép đo: F1 0{,}9955 trên đánh giá ngoại tuyến đầy đủ (Bảng~\ref{tab:summary_comparison}) và 0{,}9826 ở bản xuất huấn luyện trên mẫu con để vừa bộ nhớ một nút (Bảng~\ref{tab:edge_training}).
