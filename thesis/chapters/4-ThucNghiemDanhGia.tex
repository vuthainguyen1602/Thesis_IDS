...
\begin{table}[htbp]
\centering
\caption{Độ ổn định qua nhiều lần lấy mẫu và kiểm định ghép cặp (từ \texttt{statistical\_validity\_multiseed.csv})}
...
\Statex \qquad bảng bí danh tên cột $\mathcal{A}$ (2018 $\rightarrow$ 2017); tỉ lệ lấy mẫu $r = 0.2$; \texttt{seed} $= 42$
...
\Statex \textcolor{accOrange}{\textbf{$\rightarrow$ Loại trừ rò rỉ dữ liệu: Bước 4 và Bước 6}}
...
\State $D \gets \Call{StratifiedSample}{D, r, \text{seed}=42}$ \Comment{bảo toàn tỉ lệ lớp; còn $\sim$3{,}14 triệu luồng}
...
\begin{figure}[htbp]
    ...
    \caption{Thông lượng và độ trễ p95 của bốn cấu hình (một nút cơ sở + ba chế độ phân tán) trên cụm 2 Jetson}
