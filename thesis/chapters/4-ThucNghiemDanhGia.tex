...
Baseline (Toàn bộ) & 77 ($\approx$78 trước khi loại cổng) & \ac{xgb} & 0,9971 & 4615,5 & 2,32 \\
...
\vspace{2pt}
{\footnotesize $^{\dagger}$Kích thước đo bằng cách tuần tự hoá mô hình \emph{trên cụm huấn luyện phân tán}: thao tác \texttt{save()} ghi các phân vùng dữ liệu rừng cây trên executor đang giữ chúng, nên giá trị cỡ KB chỉ bắt được metadata cục bộ trên driver và \emph{thấp hơn thực tế}. Vì dung lượng ensemble cây phụ thuộc số cây và số nút (không phụ thuộc số đặc trưng đầu vào), mô hình \ac{xgb} SHAP Top-30 thực tế tương đương bản RF Top-30 ($\approx$2,5\,MB). Cũng vì vậy, tập đặc trưng rút gọn có thể cho mô hình \emph{lớn hơn} baseline một chút (2,52 so với 2,32\,MB): ít ứng viên tách hơn khiến cây cần nhiều nút hơn để đạt cùng độ tinh khiết. Phép đo tin cậy là bản xuất khẩu đơn nút của pipeline triển khai (Mục kết quả biên).}
\end{table}
...
Việc tinh chỉnh giúp các mô hình cây đạt F1 quanh mức 0,995 (Bảng~\ref{tab:exp2_ml_metrics}); mức cải thiện so với cấu hình cố định là nhỏ do bài toán nhị phân trong-miền đã gần bão hoà, phù hợp với nhận định ở mục kiểm định thống kê.
...
Đặc trưng \texttt{destination\_port} là một nguồn rò rỉ nhãn đã được ghi nhận trên CICIDS2017: mỗi loại tấn công được sinh nhằm vào những cổng dịch vụ cố định nên cổng đích tương quan mạnh với nhãn, và khi được giữ lại nó thường nằm trong nhóm đặc trưng quan trọng nhất theo SHAP~\cite{lanvin2023errors}. Để định lượng ảnh hưởng, luận văn thực hiện phân tích loại bỏ (ablation) giữ/loại cổng với cùng một cấu hình Random Forest cố định (Bảng~\ref{tab:leakage_ablation}). Điều đáng lưu ý là với bài toán \emph{nhị phân trong-miền}, sau khi đã loại trùng lặp luồng ở mức đặc trưng, ảnh hưởng riêng của \texttt{destination\_port} lên F1 chỉ còn \emph{nhỏ} (chênh khoảng $0{,}001$ điểm F1) --- bởi tác vụ nhị phân trên CICIDS2017 vốn gần mức bão hoà và nguồn rò rỉ \emph{chi phối} (các luồng trùng lặp) đã được xử lý trước bằng bước loại bỏ trùng lặp. Điều này \emph{không} có nghĩa rò rỉ vô hại: giá trị của quy trình loại-trừ-rò-rỉ thể hiện rõ hơn ở đánh giá \emph{đa lớp} với các lớp tấn công hiếm (Mục~\ref{sec:multiclass}) và ở \emph{tổng quát hoá liên bộ dữ liệu} (Mục~\ref{sec:cross-dataset}), nơi các ``lối tắt'' rò rỉ không còn giúp mô hình. Pipeline loại bỏ cả \texttt{destination\_port} lẫn \texttt{source\_port}; phân tích ablation tập trung vào \texttt{destination\_port} vì đây là cổng chi phối.
...
Baseline (All Features) & \ac{xgb} & 0,99734 $\pm$ 0,00007 & [0,99722; 0,99746] & \multirow{2}{*}{0,031} & \multirow{2}{*}{1,94} \\
...
Do CSE-CIC-IDS2018~\cite{CSECICIDS2018} có quy mô rất lớn ($\sim$16 triệu luồng, $\sim$6\,GB), vượt khả năng xử lý thoải mái của thiết bị biên Jetson (8\,GB), thí nghiệm này dùng \emph{mẫu phân tầng theo nhãn} của CSE-CIC-IDS2018 (tỉ lệ 0{,}2, $\sim$3{,}14 triệu luồng lấy mẫu, còn $\sim$2{,}39 triệu sau khử trùng lặp mức đặc trưng, \texttt{seed}=42, công tắc \texttt{IDS\_SAMPLE\_FRAC}) nhằm bảo toàn tỉ lệ các lớp trong khi vừa với phần cứng; CICIDS2017 dùng $\sim$2{,}23 triệu luồng. Các con số mẫu được báo cáo tường minh để bảo đảm tính tái lập; việc mở rộng lên toàn bộ CSE-CIC-IDS2018 là hướng phát triển tiếp theo.
...
\textbf{Diễn giải.} Kết quả cho thấy một tương phản gay gắt và \emph{trung thực}: F1 cùng bộ rất cao ở cả hai chiều (0{,}9952 và 0{,}9245) nhưng F1 khác bộ sụp đổ xuống 0{,}04--0{,}05, chủ yếu do recall gần bằng không (0{,}022 và 0{,}030) trong khi precision chiều 2017$\rightarrow$2018 vẫn đạt 0{,}596 --- tức mô hình gần như không nhận ra tấn công của bộ dữ liệu kia, chứ không phải báo động loạn. Nguyên nhân là \emph{dịch chuyển phân phối đặc trưng} giữa hai testbed: khác phiên bản CICFlowMeter (thang đo và cách tính một số đặc trưng thay đổi), khác hạ tầng mạng và cấu hình dịch vụ, khác tổ hợp loại tấn công (HOIC/LOIC-HTTP của 2018 không có trong 2017). Mức sụp đổ này nhất quán với các nghiên cứu cross-dataset trên IDS dựa trên đặc trưng luồng~\cite{dhooge2020interdataset,cantone2024crossdataset}, và chính là luận cứ mạnh nhất của luận văn: \emph{con số trong-miền gần hoàn hảo --- kể cả khi đã loại rò rỉ --- không bảo đảm khả năng triển khai sang môi trường khác}; đánh giá liên bộ dữ liệu phải là một phần bắt buộc của quy trình kiểm định IDS, và các hướng thích ứng miền (chuẩn hoá theo miền, huấn luyện liên bộ, cập nhật theo drift) là điều kiện tiên quyết cho triển khai thực tế.

\subsection{Đánh giá đa lớp theo loại tấn công}
\label{sec:multiclass}
Phân loại nhị phân (Benign/Attack) trên CICIDS2017 vốn dễ tách; để bộc lộ điểm yếu thực sự, luận văn bổ sung đánh giá \emph{đa lớp theo từng loại tấn công gốc} (precision/recall/F1 từng lớp, macro-F1, weighted-F1 và ma trận nhầm lẫn), gồm lớp Benign và 14 loại tấn công của CICIDS2017. Kết quả cho thấy tương phản rất rõ giữa hai chỉ số tổng hợp: \emph{weighted-F1 $\approx 0{,}998$} (bị chi phối bởi các lớp đa số) nhưng \emph{macro-F1 chỉ $\approx 0{,}806$} --- đúng bằng chứng cho thấy độ chính xác nhị phân gần hoàn hảo đã \emph{che giấu} năng lực phát hiện kém ở các lớp hiếm. Cụ thể, các lớp đa số (Benign, DoS Hulk, DDoS, PortScan) đạt F1 rất cao ($\geq 0{,}97$), trong khi các lớp cực hiếm suy giảm nghiêm trọng: Web Attack~-- XSS recall chỉ $0{,}027$, Web Attack~-- SQL Injection F1~$=0$ (chỉ 6 mẫu), Bot recall $0{,}46$. Đây là nơi khác biệt giữa các phương pháp lựa chọn đặc trưng thể hiện rõ nhất và cũng là hướng cần cải thiện (xử lý mất cân bằng, tăng cường dữ liệu lớp hiếm). Kết quả chi tiết theo từng lớp được trình bày trong Bảng~\ref{tab:per_class}, và ma trận nhầm lẫn đa lớp tương ứng được minh họa ở Hình~\ref{fig:confusion_matrix}.
...
% cảnh báo email/Slack dùng lại ảnh giai đoạn thử nghiệm sớm (img/legacy/) — caption
% đã ghi chú tường minh; mô-đun cảnh báo không đổi giữa hai giai đoạn.
\begin{figure}[htbp]
    \centering
    ...
    {\footnotesize (b) Jetson \#2 (\texttt{jetson-nano-2}) --- bộ phân loại PySpark: thông lượng \textasciitilde6 gói/s, độ trễ suy luận \textasciitilde4\,ms, đếm tấn công theo lô}
    \caption{Giao diện terminal khi pipeline IDS hoạt động trên hai node Jetson Orin Nano Super: (a) node cổng lọc bất thường và (b) node phân loại PySpark}
...
\begin{figure}[htbp]
    ...
    \caption{Email cảnh báo tấn công được gửi qua Mailtrap trong môi trường kiểm thử (ảnh chụp từ giai đoạn thử nghiệm sớm của hệ thống; mô-đun cảnh báo và định dạng thông điệp giữ nguyên khi triển khai trên cụm Jetson)}
...
\begin{figure}[htbp]
    ...
    \caption{Thông báo cảnh báo tấn công được gửi qua Slack Incoming Webhook (ảnh chụp từ giai đoạn thử nghiệm sớm; định dạng thông điệp giữ nguyên khi triển khai trên cụm Jetson)}
...
Về tiêu chí \emph{nhận biết ràng buộc biên} (edge-aware), Decision Tree có kích thước mô hình nhỏ nhất (0,046 MB), nhỏ hơn Random Forest khoảng 96 lần và thời gian huấn luyện nhanh hơn khoảng 28 lần; F1/Accuracy cần được cân nhắc cùng các chỉ số chi phí này để chọn mô hình triển khai. (Các mô hình ứng viên biên được huấn luyện trên mẫu con phân tầng khi xuất để vừa bộ nhớ một nút, nên F1 thấp hơn chút ít so với kết quả huấn luyện phân tán đầy đủ ở Bảng~\ref{tab:summary_comparison}; độ chính xác chính thức lấy từ pha huấn luyện đầy đủ.)
...
PySpark (triển khai) & 26,8 & 397 & 27,5 & -- \\
scikit-learn         & 205,5 & 49,8 & 12,7 & 2,87 \\
ONNX Runtime         & \multicolumn{4}{c}{chưa đo (chưa cài \texttt{onnxruntime} trên ARM)} \\
\bottomrule
\end{tabular*}
\end{table}
...
\begin{itemize}
    \item \textbf{Random Forest} đạt thông lượng cao nhất (26{,}8 mẫu/giây) nhờ Spark tối ưu hóa song song trên nhiều cây, nhưng sử dụng RAM nhiều nhất (27{,}5\%) và kích thước mô hình lớn nhất ($\approx$4{,}4\,MB).
    \item \textbf{\ac{gbt}} có hiệu năng trung bình ở tất cả các chỉ số, đóng vai trò cầu nối giữa Decision Tree và Random Forest.
    \item \textbf{Decision Tree} có F1-Score 0{,}9948 (SHAP Top-30, ngoại tuyến), sử dụng RAM thấp nhất (21{,}9\%), và mô hình có kích thước rất nhỏ ($\approx$0{,}05\,MB). Mặc dù thông lượng thấp nhất (22{,}0 mẫu/giây), giá trị này vẫn đáp ứng yêu cầu giám sát thời gian thực cho mạng quy mô vừa và nhỏ.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Chất lượng phát hiện}: F1-Score 0{,}9948 -- với tập đặc trưng SHAP Top-30 đã được lựa chọn (đã loại trừ rò rỉ), một cây quyết định đơn lẻ vẫn nắm bắt được các mẫu phân loại, chỉ kém Random Forest 0{,}0007 điểm F1.
    \item \textbf{Hiệu quả tài nguyên}: Chỉ sử dụng 21{,}9\% RAM, giảm áp lực lên garbage collector của JVM và đảm bảo tính ổn định lâu dài trên thiết bị giới hạn tài nguyên.
    \item \textbf{Tính khả chuyển}: Kích thước mô hình $\approx$0{,}05\,MB cho phép triển khai trên các thiết bị IoT có bộ nhớ hạn chế hơn.
\end{enumerate}

\section{Thảo luận và So sánh tổng hợp}
Kết quả thực nghiệm cho thấy việc kết hợp lựa chọn đặc trưng dựa trên SHAP và phương pháp ensemble (Hybrid Bagging) là một hướng tiếp cận phù hợp cho hệ thống IDS trên bộ dữ liệu CICIDS2017. Dưới cấu hình đánh giá loại trừ rò rỉ dữ liệu, giải pháp đạt F1-Score 0{,}9966 (Hybrid Bagging, SHAP Top-30) đồng thời tối ưu được thời gian xử lý và dung lượng lưu trữ, phù hợp với ràng buộc của thiết bị biên.

Đặc biệt, khi triển khai trên thiết bị biên Jetson Orin Nano Super, Decision Tree với SHAP Top-30 đặc trưng đạt F1-Score 0{,}9948 ngoại tuyến (đã loại trừ rò rỉ; bản xuất khẩu trên thiết bị huấn luyện trên mẫu phân tầng 20\% đạt 0{,}981), thông lượng 22{,}0 mẫu/giây, và sử dụng 21{,}9\% RAM. Các số liệu này cho phép đánh giá tính khả thi của việc triển khai hệ thống IDS dựa trên học máy trên các thiết bị biên có tài nguyên giới hạn, mở ra hướng ứng dụng thực tế cho giám sát an ninh mạng phân tán.
