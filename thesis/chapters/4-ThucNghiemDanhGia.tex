...
\begin{figure}[!ht]
    ...
    \end{tikzpicture}}
    \caption[Cụm huấn luyện phân tán Apache Spark Standalone]{Cụm huấn luyện phân tán Apache Spark Standalone: MacBook giữ vai trò \textit{master} điều phối (không chạy executor), hai Jetson Orin Nano Super là \textit{worker}/\textit{executor} thực thi tính toán; dữ liệu Parquet được phân vùng và đồng bộ sang cả hai nút, kết quả được gom về qua shuffle/collect.}
...
\begin{table}[htbp]
\centering
\caption[Tổng quát hóa liên bộ dữ liệu CICIDS2017 và CSE-CIC-IDS2018]{Tổng quát hóa liên bộ dữ liệu giữa CICIDS2017 và CSE-CIC-IDS2018 (CSE-CIC-IDS2018: mẫu phân tầng \texttt{seed}=42; từ \texttt{cross\_dataset\_results.csv})}
...
\begin{table}[htbp]
\centering
\caption[Điểm vận hành của cổng bất thường theo ngưỡng quantile]{Điểm vận hành của cổng bất thường khi quét quantile trên tập kiểm định benign (10\% benign train, \texttt{seed}=42; từ \texttt{gate\_operating\_points.csv}). FPR ở đây là tỷ lệ luồng benign bị cổng chuyển nhầm sang bộ phân loại, không phải FPR của toàn hệ thống.}
\label{tab:gate_oc}
...
\begin{figure}[htbp]
    ...
    \caption[Giao diện terminal của pipeline IDS trên hai node Jetson]{Giao diện terminal khi pipeline IDS hoạt động trên hai node Jetson Orin Nano Super: (a) node cổng lọc bất thường và (b) node phân loại PySpark}
...
\begin{figure}[htbp]
    ...
    \caption[Email cảnh báo tấn công gửi qua Mailtrap]{Email cảnh báo tấn công được gửi qua Mailtrap trong môi trường kiểm thử (ảnh chụp từ giai đoạn thử nghiệm sớm của hệ thống; mô-đun cảnh báo và định dạng thông điệp giữ nguyên khi triển khai trên cụm Jetson)}
...
\begin{figure}[htbp]
    ...
    \caption[Dashboard Grafana tại thời điểm tải đỉnh của chế độ tách pipeline]{Dashboard Grafana tại thời điểm tải đỉnh của chế độ tách pipeline: nút cổng (\texttt{jetson-nano-1}) chỉ sử dụng khoảng 5\,\% CPU trong khi nút phân loại PySpark (\texttt{jetson-nano-2}) đạt tới 91\,\%}
...
\begin{figure}[htbp]
    ...
    \caption[Thông báo cảnh báo tấn công gửi qua Slack]{Thông báo cảnh báo tấn công được gửi qua Slack Incoming Webhook (ảnh chụp từ giai đoạn thử nghiệm sớm; định dạng thông điệp giữ nguyên khi triển khai trên cụm Jetson)}
...
\begin{table}[htbp]
\centering
\caption[Chi phí suy luận một nút: Spark, sklearn và ONNX]{Chi phí suy luận một nút trên cùng mô hình/đặc trưng (Spark vs sklearn vs ONNX), đo trên nút phân loại trong cùng điều kiện bo mạch (batch 10; năng lượng là phần \emph{hoạt động}, đã trừ idle)}
...
\begin{enumerate}
    \item \textbf{Năng lực xử lý thời gian thực}: thông lượng cao nhất (26{,}8 mẫu/giây) và độ trễ p95 thấp nhất (397\,ms). Với \ac{ids}, độ trễ phán quyết là ràng buộc trực tiếp --- một mô hình nhỏ nhưng chậm hơn 35\% về p95 sẽ kéo dài cửa sổ mà tấn công chưa bị chặn.
    ...
    \item \textbf{Tương thích nền tảng}: Random Forest là estimator gốc của Spark ML nên \texttt{PipelineModel} xuất ra nạp được trực tiếp trên board ARM64, không cần các thư viện native mà tích hợp \ac{xgb} của Spark đòi hỏi.
\end{enumerate}

Đánh đổi được chấp nhận là mức tiêu thụ bộ nhớ: Random Forest dùng 27{,}5\% RAM so với 21{,}9\% của Decision Tree và mô hình nặng $\approx$4{,}4\,MB so với 0{,}05\,MB. Trên board 8\,GB, khoảng chênh này còn dư địa lớn, nên bộ nhớ không phải là ràng buộc quyết định; Decision Tree vẫn là phương án dự phòng hợp lý nếu triển khai xuống các thiết bị \ac{iot} eo hẹp bộ nhớ hơn.
