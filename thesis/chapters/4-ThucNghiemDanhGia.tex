...
Trong chương này, luận văn trình bày kết quả đạt được khi áp dụng kỹ thuật lựa chọn đặc trưng để huấn luyện các mô hình học máy. Bên cạnh đó, quy trình tinh chỉnh siêu tham số bằng tìm kiếm lưới kết hợp kiểm tra chéo (cross-validation) cũng được trình bày, cùng với đánh giá kết quả thực nghiệm khi áp dụng các thuật toán học máy như \ac{svm}, \ac{lr}, \ac{nb}, \ac{rf}, \ac{dt}, \ac{gbt}, \ac{xgb} và \ac{mlp} cả trong trường hợp có và không sử dụng kỹ thuật giảm chiều dữ liệu. Ngoài ra, luận văn cũng xác định và trình bày các giá trị tham số tối ưu cho từng thuật toán thông qua phương pháp tìm kiếm theo lưới kết hợp với kiểm tra chéo (cross-validation). Dựa trên các kết quả thu được, luận văn sẽ làm rõ vấn đề liệu việc giảm chiều dữ liệu có thực sự cần thiết hay không. Cuối cùng, luận văn đánh giá hiệu quả của các mô hình \ac{el}, sau đó so sánh với các thuật toán học máy truyền thống, từ đó trả lời câu hỏi: \textit{Liệu có nên sử dụng \ac{el} so với các mô hình học máy truyền thống khi làm việc với dữ liệu \ac{iot} hay không?}
...
\begin{figure}[!ht]
    ...
    \begin{tikzpicture}[
        ...
        work/.style  ={draw=green!55!black,  thick, rounded corners, fill=green!8,  text width=5.6cm,  minimum height=2.35cm, inner sep=5pt},
        store/.style ={draw=blue!55,         thick, rounded corners, fill=blue!5,   text width=5.6cm,  minimum height=0.85cm, inner sep=3pt},
        ...
        \node[work] (w1) at (-3.6,-4.15) {\textbf{Jetson Orin Nano Super \#1}\\[1pt]
            Spark \textbf{Worker} + \textbf{Driver}\\[2pt]
            Executor: cores=4, mem=3\,GB\\
            Phân vùng dữ liệu (partitions)\\
            swap 8\,GB (NVMe)};
        \node[work] (w2) at ( 3.6,-4.15) {\textbf{Jetson Orin Nano Super \#2}\\[1pt]
            Spark \textbf{Worker}\\[2pt]
            Executor: cores=4, mem=3\,GB\\
            Phân vùng dữ liệu (partitions)\\
            swap 8\,GB (NVMe)};
        \node[store] (s1) at (-3.6,-6.35) {Parquet (đồng bộ)};
        \node[store] (s2) at ( 3.6,-6.35) {Parquet (đồng bộ)};
        \draw[dn] (m.south -| w1) -- node[lbl]{giao tác vụ\\ + partition} (w1.north);
        \draw[dn] (m.south -| w2) -- node[lbl]{giao tác vụ\\ + partition} (w2.north);
        \draw[up] (w1.west) -- ++(-0.7,0) node[lbl,pos=1,anchor=east,xshift=-1pt]{gom kết quả\\ (shuffle/collect)} |- (m.west);
        \draw[up] (w2.east) -- ++( 0.7,0) |- (m.east);
        \draw[dn] (w1.south) -- (s1.north);
        \draw[dn] (w2.south) -- (s2.north);
    \end{tikzpicture}
    \caption{Cụm huấn luyện phân tán Apache Spark Standalone: MacBook giữ vai trò \textit{master} điều phối (không chạy executor), hai Jetson Orin Nano Super là \textit{worker}/\textit{executor} thực thi tính toán; dữ liệu Parquet được phân vùng và đồng bộ sang cả hai nút, kết quả được gom về qua shuffle/collect.}
...
\vspace{1em}
Bảng \ref{tab:model_parameters} tóm tắt thiết lập tham số của các mô hình học máy được sử dụng trong nghiên cứu này. Trong đó, \ac{xgb} và MLP được tinh chỉnh bằng Grid Search kết hợp kiểm tra chéo (Kịch bản 3); các mô hình còn lại dùng bộ tham số cố định từ trước, lựa chọn dựa trên các nghiên cứu gần đây và thực nghiệm sơ bộ, nhằm bảo đảm khác biệt quan sát được giữa các phương pháp giảm chiều phản ánh đúng phương pháp thay vì mức độ tinh chỉnh riêng từng mô hình.
...
Trong kịch bản này, toàn bộ tập đặc trưng số (khoảng 78 đặc trưng trước khi loại cổng; số chính xác sau khi loại \texttt{destination\_port}/\texttt{source\_port} là \ph{}) được đưa vào mô hình mà chưa qua bất kỳ bước lựa chọn đặc trưng hay giảm chiều nào. Mục tiêu là thiết lập một mức hiệu năng tham chiếu cho các thử nghiệm tiếp theo.
...
\begin{table}[htbp]
\centering
\caption{So sánh hiệu năng tốt nhất của các phương pháp lựa chọn đặc trưng và giảm chiều}
\label{tab:summary_comparison}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lccccc@{}}
\toprule
\textbf{Phương pháp} & \textbf{Đặc trưng} & \textbf{Mô hình tốt nhất} & \textbf{F1-Score} & \textbf{Thời gian huấn luyện (s)} & \textbf{Dung lượng (MB)} \\ \midrule
...
Ngoài ra, sự so sánh trực tiếp giữa hai phương pháp lựa chọn đặc trưng dựa trên độ quan trọng của \ac{rf} và \ac{shap} được chi tiết hóa trong Bảng \ref{tab:rf_vs_shap}.

\begin{table}[htbp]
\centering
\caption{So sánh hiệu năng lựa chọn đặc trưng: \ac{rf} Importance vs \ac{shap} Importance (Top-30)}
...
Sau khi chọn được bộ đặc trưng tối ưu từ \ac{shap}, hai mô hình \ac{xgb} và MLP được tinh chỉnh tham số thông qua Grid Search kết hợp Cross-validation; các mô hình còn lại giữ nguyên bộ tham số cố định ở Bảng~\ref{tab:model_parameters} (xem lý do ở đầu mục).
% (Ghi chú nội bộ: điền số liệu từ results/ml_03_hyperparameter_tuning/.)
Kết quả tinh chỉnh được kỳ vọng cải thiện F1-Score của MLP từ \ph{} lên \ph{}, và \ac{xgb} đạt độ ổn định cao hơn trên tập kiểm thử độc lập.
...
Hình~\ref{fig:shap_summary} xếp hạng 30 đặc trưng quan trọng nhất theo giá trị trung bình tuyệt đối của SHAP (mean $|\text{SHAP}|$): thanh càng dài, đặc trưng càng ảnh hưởng lớn đến quyết định phân loại xét trên toàn bộ mẫu. Để tránh rò rỉ, giá trị SHAP được tính trên một \emph{mẫu phân tầng theo lớp lấy từ tập huấn luyện} (không phải tập kiểm thử); mẫu này được biến đổi qua đúng bộ chuẩn hoá (StandardScaler) đã fit trên tập huấn luyện trước khi tính SHAP, bảo đảm không gian đầu vào của phép giải thích trùng khớp với không gian đầu vào mà mô hình đã học.

\begin{figure}[ht]
    \centering
    \IfFileExists{img/shap_summary_beeswarm.png}{\includegraphics[width=0.92\linewidth]{img/shap_summary_beeswarm.png}}{\fbox{\parbox{0.9\linewidth}{\centering\vspace{1.4cm}\ph{Biểu đồ SHAP beeswarm --- chạy ml\_06\_feature\_selection\_shap.py rồi \texttt{thesis/collect\_figures.sh}}\vspace{1.4cm}}}}
    \caption{Biểu đồ SHAP Summary (beeswarm): phân bố và hướng tác động của từng đặc trưng}
...
Bên cạnh đó, giá trị trung bình tuyệt đối của SHAP (mean $|\text{SHAP}|$) được dùng để xếp hạng mức quan trọng tổng thể. Trên tập đặc trưng đã loại trừ rò rỉ, các đặc trưng có mean$|\text{SHAP}|$ lớn nhất là: \ph{đặc trưng 1}, \ph{đặc trưng 2}, \ph{đặc trưng 3}, \ph{đặc trưng 4}.
...
\begin{table}[htbp]
\centering
\caption{Phân tích loại bỏ (ablation) rò rỉ: ảnh hưởng của đặc trưng \texttt{destination\_port} (Random Forest cố định a priori: \texttt{numTrees}=200, \texttt{maxDepth}=15, có trọng số lớp; nhị phân)}
...
Cả hai lượt dùng cùng một cấu hình Random Forest cố định từ trước (200 cây, độ sâu 15, trọng số lớp --- khớp Bảng~\ref{tab:model_parameters}) để cô lập đúng ảnh hưởng của đặc trưng rò rỉ. Mức chênh lệch F1 giữa hai hàng chính là phần hiệu năng \emph{bị thổi phồng} do rò rỉ. Mọi kết quả khác trong luận văn dùng cấu hình đã loại cổng.
...
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lccc@{}}
\toprule
\textbf{Thuật toán} & \textbf{Thời gian huấn luyện (s)} & \textbf{Thời gian dự đoán (s)} & \textbf{Kích thước (MB)} \\ \midrule
...
Phân loại nhị phân (Benign/Attack) trên CICIDS2017 vốn dễ tách; để bộc lộ điểm yếu thực sự, luận văn bổ sung đánh giá \emph{đa lớp theo từng loại tấn công} (precision/recall/F1 từng lớp, macro-F1 và ma trận nhầm lẫn). Để bảo đảm đủ mẫu cho các lớp hiếm, 14 loại tấn công gốc được gộp theo họ tấn công (ví dụ các biến thể DoS/DDoS gộp thành một nhóm, ba biến thể Web Attack gộp thành một nhóm), cùng với lớp Benign; sơ đồ ánh xạ nhãn gốc $\rightarrow$ nhóm lớp được ghi trong mã nguồn tiền xử lý. Các lớp đa số (DoS/DDoS, PortScan) đạt recall cao, trong khi các lớp hiếm (Heartbleed, Infiltration, Web Attack) có recall thấp hơn rõ rệt --- đây là nơi các phương pháp lựa chọn đặc trưng khác biệt nhất. Kết quả chi tiết theo từng lớp được trình bày trong Bảng~\ref{tab:per_class}, và ma trận nhầm lẫn đa lớp tương ứng được minh họa ở Hình~\ref{fig:confusion_matrix}.
...
\textbf{Đặc tả cổng bất thường (operating point).} Cổng autoencoder là một bộ lọc \emph{điều chỉnh được} chứ không phải một điểm cố định: nâng ngưỡng giúp giảm tải (offload) nhiều lưu lượng bình thường hơn khỏi Spark (tăng tỷ lệ bỏ qua tại cổng, gate-skip) nhưng có thể giảm recall trên lớp tấn công. Ngưỡng của cổng được chọn theo phân vị (quantile) của sai số tái tạo tính trên một \emph{tập kiểm định benign} --- 10\% tách từ phần lưu lượng bình thường của tập huấn luyện (\texttt{seed}=42); tập kiểm tra không tham gia việc chọn ngưỡng, tránh rò rỉ kiểu ``ngưỡng oracle''. Bảng~\ref{tab:gate_oc} trình bày đường đánh đổi giữa recall và mức giảm tải (recall--offload) khi quét các mức quantile trên tập kiểm định này; điểm vận hành được chọn theo ngân sách độ trễ/độ chính xác của triển khai.

\begin{table}[htbp]
\centering
\caption{Điểm vận hành của cổng bất thường khi quét quantile trên tập kiểm định benign (10\% benign train, \texttt{seed}=42; từ \texttt{gate\_operating\_points.csv}). FPR ở đây là tỷ lệ luồng benign bị cổng chuyển nhầm sang bộ phân loại, không phải FPR của toàn hệ thống.}
\label{tab:gate_oc}
...
% LƯU Ý: các ảnh chụp màn hình cũ (hệ Raspberry Pi) đã được chuyển vào img/legacy/
% vì KHÔNG khớp phần cứng Jetson của luận văn. Chụp lại từ cụm Jetson Orin Nano Super
% (hostname/series phải hiển thị jetson, model SHAP Top-30 mới) rồi đặt vào img/.
\begin{figure}[htbp]
    \centering
    \IfFileExists{img/pipeline_terminal.png}{\includegraphics[width=0.9\textwidth]{img/pipeline_terminal.png}}{\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}\ph{Ảnh terminal pipeline --- chụp lại trên cụm Jetson Orin Nano Super}\vspace{1.2cm}}}}
...
Về tiêu chí \emph{nhận biết ràng buộc biên} (edge-aware), Decision Tree có kích thước mô hình nhỏ nhất (\ph{}), nhỏ hơn Random Forest \ph{} lần và thời gian huấn luyện nhanh hơn \ph{} lần; F1/Accuracy cần được cân nhắc cùng các chỉ số chi phí này để chọn mô hình triển khai.
...
Phân tích kỳ vọng: chế độ \emph{pipeline split} giảm tải Spark theo tỷ lệ bỏ qua tại cổng (gate-skip; cổng lọc phần lớn lưu lượng bình thường trước khi gọi bộ phân loại); \emph{mở rộng theo chiều ngang} (horizontal scaling) tăng thông lượng khi số phân vùng (partition) Kafka $\geq 2$; \emph{Spark cluster} thêm chi phí điều phối trên phần cứng Nano nên chỉ hữu ích khi mô hình/batch vượt bộ nhớ một nút. \emph{Năng lượng mỗi suy luận} được đo riêng cho từng nút Jetson bằng \texttt{tegrastats}: công suất rail tổng được lấy mẫu suốt cửa sổ tải và tích phân theo thời gian, \emph{có trừ} đường nền công suất nghỉ (idle baseline) đo trước mỗi lần chạy, rồi chia cho số luồng đã phân loại; với chế độ pipeline split, năng lượng báo cáo là \emph{tổng năng lượng hoạt động của cả hai nút} (cổng trên Jetson~\#1 + bộ phân loại trên Jetson~\#2). Hai lưu ý trung thực: (i) đường nền idle đo một lần trước mỗi run nên chưa bắt được trôi công suất do nhiệt trong run; (ii) phần tiết kiệm nhờ gate-skip chưa trừ chi phí của chính autoencoder --- vốn chạy trên 100\% luồng.
...
\begin{itemize}
    \item \textbf{Random Forest} đạt thông lượng cao nhất (\ph{} mẫu/giây) nhờ Spark tối ưu hóa song song trên nhiều cây, nhưng sử dụng RAM nhiều nhất (\ph{}\%) và kích thước mô hình lớn nhất (\ph{}).
    \item \textbf{\ac{gbt}} có hiệu năng trung bình ở tất cả các chỉ số, đóng vai trò cầu nối giữa Decision Tree và Random Forest.
    \item \textbf{Decision Tree} có F1-Score \ph{}, sử dụng RAM thấp nhất (\ph{}\%), và mô hình có kích thước rất nhỏ (\ph{}). Mặc dù thông lượng thấp nhất (\ph{} mẫu/giây), giá trị này vẫn đáp ứng yêu cầu giám sát thời gian thực cho mạng quy mô vừa và nhỏ.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Chất lượng phát hiện}: F1-Score \ph{} -- với tập đặc trưng SHAP Top-30 đã được lựa chọn (đã loại trừ rò rỉ), cần kiểm chứng liệu một cây quyết định đơn lẻ có đủ khả năng nắm bắt các mẫu phân loại hay không.
    \item \textbf{Hiệu quả tài nguyên}: Chỉ sử dụng \ph{}\% RAM, giảm áp lực lên garbage collector của JVM và đảm bảo tính ổn định lâu dài trên thiết bị giới hạn tài nguyên.
...
Kết quả thực nghiệm cho thấy việc kết hợp lựa chọn đặc trưng dựa trên SHAP và phương pháp ensemble (Hybrid Bagging) là một hướng tiếp cận phù hợp cho hệ thống IDS trên bộ dữ liệu CICIDS2017. Dưới cấu hình đánh giá loại trừ rò rỉ dữ liệu, giải pháp đạt F1-Score \ph{} đồng thời tối ưu được thời gian xử lý và dung lượng lưu trữ, phù hợp với ràng buộc của thiết bị biên.

Đặc biệt, khi triển khai trên thiết bị biên Jetson Orin Nano Super, Decision Tree với SHAP Top-30 đặc trưng được kỳ vọng đạt F1-Score \ph{} (đã loại trừ rò rỉ), thông lượng \ph{} mẫu/giây, và sử dụng \ph{}\% RAM. Sau khi điền các số liệu trên, kết quả sẽ cho phép đánh giá tính khả thi của việc triển khai hệ thống IDS dựa trên học máy trên các thiết bị biên có tài nguyên giới hạn, mở ra hướng ứng dụng thực tế cho giám sát an ninh mạng phân tán.
