...
\end{tabular*}
\tabnote{Ba dòng của mỗi chiều đến từ \emph{cùng một lượt chạy} nên so sánh được với nhau. Mức cơ sở của lượt chạy này (0{,}0423 và 0{,}0561) là một mẫu của phân phối F1 khác bộ đã mô tả ở trên, nên khác mức cơ sở ghi ở Bảng~\ref{tab:label_budget} vốn thuộc một lượt chạy khác; điều đó không ảnh hưởng tới kết luận rút ra ở đây, vì cả hai kỹ thuật đều đưa F1 về đúng 0 bất kể mức cơ sở. Ánh xạ CORAL được kiểm chứng giải tích: sau khi ánh xạ, trung bình và hiệp phương sai của miền đích khớp miền nguồn với sai số tương đối $4\times10^{-12}$.}
\end{table}
...
\end{tabular*}
\tabnote{1\% được lấy từ \emph{tập huấn luyện} của miền đích, không bao giờ từ tập kiểm tra. Dòng ``không thích ứng'' lấy từ chính lượt chạy này để ba dòng so sánh được với nhau; mức cơ sở 0{,}2939 vì vậy là một mẫu khác của phân phối F1 khác bộ nói trên (Bảng~\ref{tab:cross_adapt} rơi vào mẫu 0{,}0423 của cùng phân phối đó). Kết luận rút ra dưới đây không phụ thuộc vào mức cơ sở cụ thể: kể cả khi so với mẫu cao nhất quan sát được (0{,}3009), mức tăng do 1\% nhãn vẫn là hơn một bậc độ lớn ở cả hai chiều.}
\end{table}
