...
\end{tabular*}
\tabnote{Cùng một lượt chạy nên các dòng so sánh được với nhau; dòng ``không thích ứng'' chiều 2017$\rightarrow$2018 trùng khít Bảng~\ref{tab:cross_dataset} tới chữ số cuối, còn chiều 2018$\rightarrow$2017 lệch $2{,}6\times10^{-3}$ do dao động giữa các lần chạy. Ánh xạ CORAL được kiểm chứng giải tích: sau khi ánh xạ, trung bình và hiệp phương sai của miền đích khớp miền nguồn với sai số tương đối $4\times10^{-12}$.}
\end{table}
...
\end{tabular*}
\tabnote{1\% được lấy từ \emph{tập huấn luyện} của miền đích, không bao giờ từ tập kiểm tra. Dòng ``không thích ứng'' lấy từ chính lượt chạy này để ba dòng so sánh được với nhau; kết luận rút ra dưới đây không phụ thuộc vào mức cơ sở cụ thể, vì mức tăng do 1\% nhãn là hơn một bậc độ lớn ở cả hai chiều.}
\end{table}
