...
Dữ liệu lớn thường được nhận diện thông qua năm đặc điểm cốt lõi: \textbf{Khối lượng} (\textit{Volume}), \textbf{Tốc độ} (\textit{Velocity}), \textbf{Tính đa dạng} (\textit{Variety}), \textbf{Tính xác thực} (\textit{Veracity}) và \textbf{Giá trị} (\textit{Value}). Đây là những yếu tố quan trọng giúp phân biệt dữ liệu lớn với các loại dữ liệu thông thường, phản ánh quy mô, tốc độ phát sinh, sự phong phú về định dạng, độ tin cậy và tiềm năng khai thác thông tin từ dữ liệu.

\vspace*{1ex}
    \begin{figure}[ht]
    \centering
    \includegraphics[scale=0.8]{img/bigdata.png}
...
Apache Spark bao gồm nhiều thành phần chính tạo thành một hệ sinh thái hoàn chỉnh, hỗ trợ đa dạng các tác vụ phân tích dữ liệu như xử lý dữ liệu tuần tự, truy vấn SQL, phân tích luồng dữ liệu thời gian thực, học máy và xử lý đồ thị.

\vspace*{1ex}
    \begin{figure}[ht]
    \centering
    \includegraphics[scale=0.7]{img/spark.png}
...
\begin{itemize}
    \item \textbf{Voting}: Với bài toán phân loại, mỗi mô hình con bỏ phiếu cho một lớp và lớp có số phiếu cao nhất sẽ là đầu ra cuối cùng (đa số phiếu hoặc trung bình xác suất). Các bước của voting được mô tả chi tiết ở \textbf{\textit{Thuật toán \ref{voting_ag1}}}.
    \item \textbf{Boosting}: Huấn luyện các mô hình tuần tự, trong đó mỗi mô hình mới tập trung vào các mẫu mà mô hình trước đã dự đoán sai (ví dụ: \ac{gbt}, AdaBoost, \ac{xgb}). Các bước của boosting được mô tả chi tiết ở \textbf{\textit{Thuật toán \ref{boosting_ag1}}}.
    \item \textbf{Bagging (Bootstrap Aggregating)}: Huấn luyện nhiều mô hình trên các tập con ngẫu nhiên (lấy mẫu có hoàn lại) của tập dữ liệu gốc, sau đó tổng hợp kết quả dự đoán (ví dụ: Random Forest). Các bước của bagging được mô tả chi tiết ở \textbf{\textit{Thuật toán \ref{bagging_ag1}}}.
\end{itemize}
