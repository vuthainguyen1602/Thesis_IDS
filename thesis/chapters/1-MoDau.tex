...
\begin{itemize}
  ...
  \item So sánh và đánh giá các mô hình học máy phổ biến như Decision Tree, Support Vector Machine, Logistic Regression, \ac{nb}, Random Forest, \ac{gbt}, \ac{xgb}, \ac{mlp} và các thuật toán Ensemble learning trong môi trường xử lý phân tán, từ đó chọn ra mô hình phù hợp nhất.
...
\begin{itemize}
  ...
  \item \textbf{RQ2}: Các mô hình học máy tổng hợp (\ac{el}) có mang lại lợi thế đáng kể so với mô hình đơn tốt nhất hay không, khi khác biệt được kiểm chứng bằng kiểm định thống kê thay vì so sánh một lần đo?
...
\begin{itemize}
  ...
  \item \textbf{Nghiên cứu về phương pháp tối ưu hóa siêu tham số, các phương pháp giảm chiều dữ liệu}: Nghiên cứu các phương pháp giảm chiều dữ liệu như \ac{pca}, \ac{shap} và Random Forest Importance nhằm giảm nhiễu và tăng độ chính xác, đồng thời tối ưu hóa siêu tham số thông qua tìm kiếm lưới (Grid Search) và kiểm tra chéo (cross-validation) giúp nâng cao hiệu năng mô hình. Các bước này được triển khai trên nền tảng Apache Spark nhằm đảm bảo khả năng xử lý dữ liệu lớn hiệu quả và mở rộng.
  \item \textbf{Nghiên cứu và đề xuất mô hình học máy, học sâu (MLP; và một autoencoder làm cổng phát hiện bất thường), học máy tổng hợp phát hiện xâm nhập sử dụng Spark Machine Learning}: Sử dụng nhiều thuật toán khác nhau như Decision Tree, Support Vector Machine, Logistic Regression, \ac{nb}, Random Forest, \ac{gbt}, \ac{xgb}, \ac{mlp} và các thuật toán Ensemble learning để xây dựng mô hình nhằm so sánh, đánh giá mô hình có hiệu năng tốt nhất.
